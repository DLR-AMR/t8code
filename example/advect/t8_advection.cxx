  ...
  for (itree = 0, idata = 0; itree < num_trees; itree++) {
    ...
    for (ielement = 0; ielement < num_elems_in_tree; ielement++, idata++) {
      ...
      for (iface = 0; iface < elem_data->num_faces; iface++) {
        ...
        t8_forest_leaf_face_neighbors (problem->forest, itree, element, &neighbors, iface, &elem_data->dual_faces[iface], &elem_data->num_neighbors[iface], &elem_data->neighs[iface], &neigh_scheme, 1);
        ...
        for (iface = 0; iface < num_faces; iface++) {
          if (elem_data->flux_valid[iface] <= 0 || adapted_or_partitioned) {
            ...
            if (adapted_or_partitioned) {
              ...
              t8_forest_leaf_face_neighbors (problem->forest, itree, elem, &neighs, iface, &elem_data->dual_faces[iface], &elem_data->num_neighbors[iface], &elem_data->neighs[iface], &neigh_scheme, 1);
