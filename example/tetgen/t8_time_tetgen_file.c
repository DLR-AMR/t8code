                                 ...
                                 int do_partition)
{
  ...
  sc_flops_start (&fi);
  cmesh = t8_cmesh_from_tetgen_file_time ((char *) prefix, do_partition, sc_MPI_COMM_WORLD,
                                       do_dup, &fi, &snapshot, stats, 0);
  if (cmesh != NULL) {
  ...
  sc_stats_print (p4est_package_id, SC_LP_STATISTICS, 1, stats, 1, 1);
}
