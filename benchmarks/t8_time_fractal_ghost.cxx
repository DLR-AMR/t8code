                      ...
                      const t8_eclass_t eclass, const int output, const int coarse, const int runs)
{
  ...
  // 0T8_ASSERT ((eclass == T8_ECLASS_QUAD || eclass == T8_ECLASS_HEX) && level_initial > 1 && 0 == level_initial % 2
  //           && 0 == level_end % 2);
  ...
  for (int run = 0; run < runs; run++) {
    ...
    // for (int level = level_initial; level <= level_end; level++) {
      ...
      time_ghost = t8_forest_profile_get_ghost_time (forest_adapt, &ghosts_sent);
      forest = forest_adapt;

      t8_productionf ("ghost time: l = %d, t = %f s, #sent = %d\n", level_end, time_ghost, ghosts_sent);
