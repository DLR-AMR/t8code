        ...
        for proc_log in range(max_proc_log+1):
                ...
                for level in range(maxlevel+1,-1,-1):
                    if (max_elem_log >= dim*level - proc_log >= min_elem_log):
                        ...
                        string += f"srun -o e{eclass}/s{scheme}/p{proc_log}_l{level}_%j ./t8_time_search -s{scheme} -e{eclass} -l{level} -r{repititions}\n"

                file = Path(f"e{eclass}/s{scheme}/scripts/p{proc_log}.sh")
...
for eclass in eclasses:
    for scheme in schemes:
        for proc_log in range(max_proc_log+1):
            subprocess.call(f"sbatch e{eclass}/s{scheme}/scripts/p{proc_log}.sh", shell=True)
