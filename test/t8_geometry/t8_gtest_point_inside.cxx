...
class geometry_point_inside: public testing::TestWithParam<std::tuple<t8_eclass, int, int>> {
 protected:
  ...
  SetUp () override
  {
    ...
    if (use_axis_aligned_geom && (eclass == T8_ECLASS_LINE || eclass == T8_ECLASS_QUAD || eclass == T8_ECLASS_HEX || eclass == T8_ECLASS_TRIANGLE || eclass == T8_ECLASS_TET || eclass == T8_ECLASS_PRISM)) {
      cmesh = t8_cmesh_new_hypercube (eclass, sc_MPI_COMM_WORLD, 1, 1, 1, use_axis_aligned_geom);
    }
    else {
      cmesh = t8_cmesh_new_hypercube (eclass, sc_MPI_COMM_WORLD, 1, 1, 0, false);
    }
  }
  void
  TearDown () override
...
TEST_P (geometry_point_inside, test_point_inside)
{
   ...
   */
  for (int itree = 0; itree < t8_cmesh_get_num_trees (cmesh); ++itree) {
    double *const tree_vertices = t8_cmesh_get_tree_vertices (cmesh, itree);
    const int num_vertices = (use_axis_aligned_geom && (eclass == T8_ECLASS_TRIANGLE || eclass == T8_ECLASS_TET || eclass == T8_ECLASS_PRISM || eclass == T8_ECLASS_HEX || eclass == T8_ECLASS_QUAD))
                               ? 2
  ...
  for (t8_locidx_t itree = 0; itree < num_trees; ++itree) {
    ...
    for (t8_locidx_t ielement = 0; ielement < num_elements; ++ielement) {
      ...
      t8_forest_element_points_inside (forest, itree, element, test_point, total_points, point_is_recognized_as_inside,
                                       tolerance);
      for (int ipoint = 0; ipoint < num_points; ipoint++) {
...
#else
INSTANTIATE_TEST_SUITE_P (t8_gtest_point_inside, geometry_point_inside,
                          testing::Combine (testing::Range (T8_ECLASS_LINE, T8_ECLASS_COUNT), testing::Range (0, 3),
                                            testing::Range (0, 2)),
                          print_test);
#endif
