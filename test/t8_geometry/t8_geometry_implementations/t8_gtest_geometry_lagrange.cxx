...
TEST (test_geometry_lagrange, incompatible_geometry)
{
  ...
  /* Should return 0 since the t8_geometry_lagrange geometry is compatible with quads. */
   ...
   * Should return 2 since the t8_geometry_lagrange geometry is not compatible with prisms. */
  ...
  t8_cmesh_register_geometry<t8_geometry_lagrange> (cmesh);
  /* Check validity after committing to circumvent the assertion.
   * Should return 2 since the maximum polynomial degree is exceeded. */
  ASSERT_EQ (t8_cmesh_validate_geometry (cmesh), 2);
  t8_cmesh_destroy (&cmesh);
}
