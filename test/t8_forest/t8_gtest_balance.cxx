                                       ...
                                       const int num_elements, t8_element_t *elements[])
{
  ...
  if(std::find(adapt_data->trees_to_refine.begin(), adapt_data->trees_to_refine.end(), gtree_id) != adapt_data->trees_to_refine.end() &&
...
static
t8_forest_t
t8_gtest_obtain_forest_for_balance_tests(const std::vector<t8_gloidx_t>& trees_to_refine, int additional_refinement = 0)
{
...
static
bool
t8_gtest_check_custom_balanced_forest(t8_forest_t balanced_forest, const std::array<int, gtest_balance::kNumTrees>& expected_elem_level_per_tree)
{
  ...
  for (t8_locidx_t tree_id = 0; tree_id < num_local_trees; ++tree_id)
  {
    ...
    const t8_eclass_scheme_c *ts = t8_forest_get_eclass_scheme (balanced_forest, t8_forest_get_tree_class(balanced_forest, tree_id));
...
TEST(gtest_balance, balance_consistency_test)
{
  ...
  EXPECT_TRUE(t8_forest_is_equal(balanced_forest, already_balanced_forest) == 1);
