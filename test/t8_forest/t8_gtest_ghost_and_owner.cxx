...
t8_test_gao_check (t8_forest_t forest)
{
  ...
  for (t8_locidx_t itree = 0, lelement_id = 0; itree < num_ghost_trees;
       itree++) {
    ...
    t8_gloidx_t         gtreeid =
      t8_forest_ghost_get_global_treeid (forest, itree);
    t8_eclass_t         eclass =
      t8_forest_ghost_get_tree_class (forest, itree);
    for (t8_locidx_t ielem = 0; ielem < num_elems_in_tree;
         ielem++, lelement_id++) {
      ...
      t8_element_t       *ghost_element =
        t8_forest_ghost_get_element (forest, itree, ielem);
      int                 is_owner =
