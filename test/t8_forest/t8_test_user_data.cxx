 ...
 */
static void
t8_test_user_data (sc_MPI_Comm mpic)
{
  /* Build a forest */
  t8_cmesh_t          cmesh =
    t8_cmesh_new_hypercube (T8_ECLASS_TRIANGLE, mpic, 0, 0, 0);
  t8_scheme_cxx_t    *scheme = t8_scheme_new_default_cxx ();
  t8_forest_t         forest =
    t8_forest_new_uniform (cmesh, scheme, 1, 0, mpic);
  /* Define user data */
  double              data = 42.42;
  double             *get_data;
  int                 data2 = 42;
  ...
  SC_CHECK_ABORT (get_data == &data,
                  "Forest returned wrong user data pointer.");
  /* Check whether data was unchanged. */
  SC_CHECK_ABORT (*get_data == 42.42, "User data value has changed.");
  ...
  /* Check whether retrieved pointer points to data. */
  SC_CHECK_ABORT (get_data2 == &data2,
                  "Forest returned wrong user data pointer.");
  /* Check whether data was unchanged. */
  SC_CHECK_ABORT (*get_data2 == 42, "User data value has changed.");
 ...
 */
static void
t8_test_user_function (sc_MPI_Comm mpic)
{
  /* Build a forest */
  t8_cmesh_t          cmesh =
    t8_cmesh_new_hypercube (T8_ECLASS_TRIANGLE, mpic, 0, 0, 0);
  ...
  /* Check whether the function pointer is correct. */
  SC_CHECK_ABORT (funpointer_second == &t8_test_function_second,
                  "Forest returned wrong user function pointer.");
