...
TEST_P (forest_half_neighbors, test_half_neighbors)
{
  ...
  for (t8_locidx_t itree = 0; itree < t8_forest_get_num_local_trees (forest); itree++) {
    for (t8_locidx_t ielement = 0; ielement < t8_forest_get_tree_num_elements (forest, itree); ielement++) {
      ...
      for (int face = 0; face < ts->element_get_num_faces (eclass, element); face++) {
        ...
        const int num_face_neighs = ts->element_get_num_face_children (eclass, element, face);
        t8_element_t **half_neighbors = T8_ALLOC (t8_element_t *, num_face_neighs);
        ts->element_new (eclass, num_face_neighs, half_neighbors);
        ...
        ts->element_new (neigh_class, 1, &neighbor);
        ...
        if (neigh_tree > 0) {
          ...
          T8_ASSERT (num_face_neighs == ts->element_get_num_face_children (neigh_class, neighbor, dual_face));
          t8_element_t **neighbor_face_children = T8_ALLOC (t8_element_t *, num_face_neighs);
          ts->element_new (neigh_class, num_face_neighs, neighbor_face_children);
          int *child_ids = T8_ALLOC (int, num_face_neighs);
          ts->element_get_children_at_face (neigh_class, neighbor, dual_face, neighbor_face_children, num_face_neighs,
                                            child_ids);
          ...
          for (int ineigh = 0; ineigh < num_face_neighs; ineigh++) {
            EXPECT_ELEM_EQ (ts, neigh_class, neighbor_face_children[ineigh], half_neighbors[ineigh])
              << "ineigh = " << ineigh << " face = " << face;
          }
          ts->element_destroy (neigh_class, num_face_neighs, neighbor_face_children);
          T8_FREE (child_ids);
          T8_FREE (neighbor_face_children);
        }
        ts->element_destroy (neigh_class, 1, &neighbor);
        ts->element_destroy (neigh_class, num_face_neighs, half_neighbors);
