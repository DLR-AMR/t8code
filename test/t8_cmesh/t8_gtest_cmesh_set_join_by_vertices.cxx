...
static void test_with_cmesh(t8_cmesh_t cmesh) {
  ...
  for (int this_itree = 0; this_itree < ntrees; this_itree++) {
    ...
    for (int this_iface = 0; this_iface < this_nfaces; this_iface++) {
      const int conn_dual_itree = conn[T8_I3(ntrees,T8_ECLASS_MAX_FACES,3,this_itree,this_iface,0)];
      const int conn_dual_iface = conn[T8_I3(ntrees,T8_ECLASS_MAX_FACES,3,this_itree,this_iface,1)];
      const int conn_orientation = conn[T8_I3(ntrees,T8_ECLASS_MAX_FACES,3,this_itree,this_iface,2)];

      int cmesh_dual_iface;
      int cmesh_orientation;

      t8_locidx_t cmesh_dual_itree = t8_cmesh_get_face_neighbor (cmesh,
                                                this_itree,
                                                this_iface,
                                                &cmesh_dual_iface,
                                                &cmesh_orientation);
      ...
      if (cmesh_dual_itree > -1) {
        ...
        for (int this_iface_vert = 0; this_iface_vert < this_nface_verts; this_iface_vert++) {
          ...
          for (int dual_iface_vert = 0; dual_iface_vert < dual_nface_verts; dual_iface_vert++) {
            ...
            for (int icoord = 0; icoord < T8_ECLASS_MAX_DIM; icoord++) {
              const double this_face_vert = this_vertices[T8_I2(this_nface_verts,T8_ECLASS_MAX_DIM,this_ivert,icoord)];
              const double dual_face_vert = dual_vertices[T8_I2(dual_nface_verts,T8_ECLASS_MAX_DIM,dual_ivert,icoord)];
