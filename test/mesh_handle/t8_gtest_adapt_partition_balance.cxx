...
TEST (t8_gtest_handle_adapt, compare_with_forest)
{
  ...
  mesh_handle.set_adapt (
    mesh_class::mesh_adapt_callback_wrapper<dummy_user_data> (adapt_callback_sphere<mesh_class>, user_data));
  mesh_handle.commit ();
  // Adapt forest classically.
  forest = t8_forest_new_adapt (forest, forest_adapt_callback_sphere, 0, 0, &user_data);
  ...
  t8_forest_set_adapt (forest_compare, forest, forest_adapt_callback_sphere, false);
  ...
  mesh_handle.set_adapt (adapt_callback_refine_second<mesh_class>);
  mesh_handle.commit ();
