...
class all_cmeshes_with_num_elem {
    ...
    all_cmeshes_with_num_elem (int creator, sc_MPI_Comm comm, int num_elems) :
                                ...
                                create_func(cmeshes_with_num_elems[creator]),
                                comm(comm){}
    ...
    all_cmeshes_with_num_elem( const all_cmeshes_with_num_elem &other) :
                                ...
                                current_creator(other.current_creator),
                                create_func(other.create_func),
                                comm(other.comm),
                                cmesh(NULL){}

    /* overloading the < operator for gtest-ranges */
    bool operator<(const all_cmeshes_with_num_elem &other){
        t8_debugf("[D] gen_a %i, num_a %i, gen_b %i, num_b %i\n", current_creator, num_elems, other.current_creator, other.num_elems);
    ...
    all_cmeshes_with_num_elem operator+(const all_cmeshes_with_num_elem &step){
        ...
        if(current_creator > 0 && num_elems <= 2) num_elems = 3;
        t8_debugf("[D] next: %i %i\n", current_creator, num_elems);
