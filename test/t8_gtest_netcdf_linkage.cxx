 ...
 * the disk. */
  ...
  /* Check for error */
  EXPECT_EQ (nc_error, NC_NOERR) << "netcdf error when creating in memory file: " << nc_strerror(nc_error);

  /* Close the file */
  nc_error = nc_close (ncid);
  /* Check for error */
  EXPECT_EQ (nc_error, NC_NOERR) << "netcdf error when closing in memory file: " << nc_strerror(nc_error);
