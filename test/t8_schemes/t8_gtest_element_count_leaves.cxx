...
TEST_P (class_element_leaves, test_element_count_leaves_less_level)
{
  ...
  for (int level = 0; level <= maxlevel; ++level) {
    ...
    const t8_gloidx_t leaf_count_same_level
      = scheme->element_count_leaves (static_cast<t8_eclass_t> (scheme_id), element, level);
    /* Check if equals 1 */
    ...
    for (lower_levels = level - 1; lower_levels >= 0; --lower_levels) {
      ...
      const t8_gloidx_t leaf_count
        = scheme->element_count_leaves (static_cast<t8_eclass_t> (scheme_id), element, lower_levels);
      /* Check if equals 0 */
