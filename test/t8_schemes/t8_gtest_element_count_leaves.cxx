...
class class_element_leaves: public testing::TestWithParam<int> {
 protected:
  void
  SetUp () override
  {
    scheme_id = GetParam ();
    eclass = scheme->get_eclass_scheme_eclass (static_cast<t8_eclass_t>(scheme_id));
  }
  void
  TearDown () override
  {
...
{
  const int maxlevel = scheme->get_maxlevel (static_cast<t8_eclass_t>(scheme_id));
  ...
  for (int level = 0; level <= maxlevel; ++level) {
    const t8_gloidx_t leaf_count = scheme->count_leaves_from_root (static_cast<t8_eclass_t>(scheme_id), level);
...
TEST_P (class_element_leaves, test_element_count_leaves_less_level)
{
  ...
  for (int level = 0; level <= maxlevel; ++level) {
    ...
    scheme->element_set_linear_id (static_cast<t8_eclass_t>(scheme_id), element, level, 0);
    /* Count the leaves of this element */
    const t8_gloidx_t leaf_count_same_level = scheme->element_count_leaves (static_cast<t8_eclass_t>(scheme_id), element, level);
    /* Check if equals 1 */
    ...
    for (lower_levels = level - 1; lower_levels >= 0; --lower_levels) {
      ...
      const t8_gloidx_t leaf_count = scheme->element_count_leaves (static_cast<t8_eclass_t>(scheme_id), element, lower_levels);
      /* Check if equals 0 */
...
TEST_P (class_element_leaves, test_element_count_leaves_one_level)
{
  ...
  for (int level = 1; level < maxlevel; ++level) {
    ...
    scheme->element_set_linear_id (static_cast<t8_eclass_t>(scheme_id), element, level - 1, 0);
    /* Count the leaves of this element */
    const t8_gloidx_t leaf_count = scheme->element_count_leaves (static_cast<t8_eclass_t>(scheme_id), element, level);
    /* Compute the number of children of the element */
    const int number_of_children = scheme->element_get_num_children (static_cast<t8_eclass_t>(scheme_id), element);
