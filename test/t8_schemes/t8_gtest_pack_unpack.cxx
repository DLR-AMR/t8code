...
class class_test_pack: public TestDFS {
  ...
  check_element () override
  {
    ...
    int pack_size;
    int num_children = ts->element_get_num_children (tree_class, element);
    ts->element_MPI_Pack_size (tree_class, count, comm, &pack_size);
    pack_size *= (num_children + 1);
    char *sendbuf = T8_ALLOC (char, pack_size);

    /* pack data */
    ts->element_MPI_Pack (tree_class, &element, count, sendbuf, pack_size, &position, comm);
    t8_element_t **children = T8_ALLOC (t8_element_t *, num_children);
    ts->element_new (tree_class, num_children, children);
    ts->element_get_children (tree_class, element, num_children, children);
    ts->element_MPI_Pack (tree_class, children, num_children, sendbuf, pack_size, &position, comm);
    ...
    /* Unpack data */
    position = 0;
    ts->element_MPI_Unpack (tree_class, recvbuf, recvBufferSize, &position, &element_compare, count, comm);
    t8_element_t **children_compare = T8_ALLOC (t8_element_t *, num_children);
    ts->element_new (tree_class, num_children, children_compare);
    ts->element_MPI_Unpack (tree_class, recvbuf, recvBufferSize, &position, children_compare, num_children, comm);
    ...
    EXPECT_ELEM_EQ (ts, tree_class, element, element_compare);
    for (int ichild = 0; ichild < num_children; ichild++) {
      EXPECT_ELEM_EQ (ts, tree_class, children[ichild], children_compare[ichild]);
    }
    ts->element_destroy (tree_class, num_children, children);
    ts->element_destroy (tree_class, num_children, children_compare);
    T8_FREE (children);
 ...
 protected:
  ...
  SetUp () override
  {
    ...
    ts->element_new (tree_class, 1, &element_compare);
  ...
  {
    /* Destroy element */
    ts->element_destroy (tree_class, 1, &element_compare);
