...
class class_child_parent_face: public TestDFS {
  ...
  check_element () override
  {
    ...
    for (int iface = 0; iface < num_faces; iface++) {
      ...
      scheme->element_get_children_at_face (static_cast<t8_eclass_t> (scheme_id), element, iface, children,
      ...
      for (int ifacechild = 0; ifacechild < num_face_children; ifacechild++) {
        ...
        const int childface
          = scheme->element_face_get_child_face (static_cast<t8_eclass_t> (scheme_id), element, iface, ifacechild);
        ASSERT_NE (childface, -1);
        /* Determine the parentface corresponding to the childface */
        const int parentface = scheme->element_face_get_parent_face (static_cast<t8_eclass_t> (scheme_id),
                                                                     children[ifacechild], childface);
