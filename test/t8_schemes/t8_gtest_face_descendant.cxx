...
{
  const int num_children_at_face = ts->element_get_num_face_children (tree_class, element, iface);

  int *child_indices = T8_ALLOC (int, num_children_at_face);
  t8_element_t **children = T8_ALLOC (t8_element_t *, num_children_at_face);
  ts->element_new (tree_class, num_children_at_face, children);

  ts->element_copy (tree_class, element, face_desc);
  const int level = ts->element_get_level (tree_class, element);
  for (int ilevel = level; ilevel < desc_level; ilevel++) {
    EXPECT_EQ (ts->element_get_num_face_children (tree_class, element, iface), num_children_at_face);
    /* Compute child_id of the test_child_id-th child. */
    ts->element_get_children_at_face (tree_class, face_desc, iface, children, num_children_at_face, child_indices);
    ...
    ts->element_get_child (tree_class, face_desc, child_id, face_desc);
  }
  ts->element_destroy (tree_class, num_children_at_face, children);
  T8_FREE (children);
...
class class_descendant: public TestDFS {
  ...
  check_element () override
  {
    ...
    for (int ilevel = level + 1; ilevel < max_test_lvl; ilevel++) {
      for (int jface = 0; jface < num_faces; jface++) {
        ...
        ts->element_construct_first_descendant_face (tree_class, element, jface, scheme_face_desc, ilevel);
        /* Compare the manually computed child with the result of t8_element_first_descendant_face. */
        EXPECT_ELEM_EQ (ts, tree_class, scheme_face_desc, manual_face_desc);

        t8_test_manual_first_last_face_descendant (ts, element, tree_class, jface, ilevel, 1, manual_face_desc);
        ts->element_construct_last_descendant_face (tree_class, element, jface, scheme_face_desc, ilevel);
 ...
 protected:
  ...
  {
    dfs_test_setup ();
    max_test_lvl = ts->get_maxlevel (tree_class);
    ts->element_new (tree_class, 1, &manual_face_desc);
    ts->element_new (tree_class, 1, &scheme_face_desc);
  }
  void
  TearDown () override
  {
    ts->element_destroy (tree_class, 1, &manual_face_desc);
    ts->element_destroy (tree_class, 1, &scheme_face_desc);
    dfs_test_teardown ();
  }
  int max_test_lvl;
