                              ...
                              t8_scheme *scheme, const int scheme_id)
{
  ...
  for (int iface = 0; iface < num_faces; iface++) {
    ...
    EXPECT_TRUE (scheme->element_is_equal (static_cast<t8_eclass_t> (scheme_id), child, element))
      << "Got a false neighbor.";
...
TEST_P (face_neigh, check_not_inside_root)
{
  ...
  for (int iface = 0; iface < num_faces; iface++) {
    ...
    const int num_children
      = scheme->element_get_num_face_children (static_cast<t8_eclass_t> (scheme_id), element, iface);
    ...
    scheme->element_get_children_at_face (static_cast<t8_eclass_t> (scheme_id), element, iface, children, num_children,
                                          child_indices);
    ...
    for (int jchild = 0; jchild < num_children; jchild++) {
      ...
      const int face_contact
        = scheme->element_face_get_child_face (static_cast<t8_eclass_t> (scheme_id), element, iface, jchild);

      scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), element, child_id, child);
      int face_num;
      int inside = scheme->element_get_face_neighbor_inside (static_cast<t8_eclass_t> (scheme_id), child, neigh,
                                                             face_contact, &face_num);
