      ...
      for (check_lvl_a = lvl + 1; check_lvl_a < elem_max_level; check_lvl_a++) {
        ...
        for (check_lvl_b = lvl + 1; check_lvl_b < elem_max_level; check_lvl_b++) {
          ...
          if (tree_class == T8_ECLASS_VERTEX) {
            ...
            EXPECT_EQ (
              scheme->element_get_level (static_cast<t8_eclass_t>(scheme_id), check),
              SC_MIN (scheme->element_get_level (static_cast<t8_eclass_t>(scheme_id), desc_a), scheme->element_get_level (static_cast<t8_eclass_t>(scheme_id), desc_b)));
          }
          else {
    ...
    for (j = 0; j < num_children_b; j++) {
      ...
      if (!scheme->element_is_equal (static_cast<t8_eclass_t>(scheme_id), check_nca, check)) {
        ...
        t8_debugf ("id of desc_a: %li, level: %i\n",
                   static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t>(scheme_id), desc_a, level_a)), level_a);
        t8_debugf ("id of desc_b: %li, level: %i\n",
                   static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t>(scheme_id), desc_b, level_b)), level_b);
        ...
        t8_debugf ("id of the correct nca: %li, level: %i\n",
                   static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t>(scheme_id), check_nca, level_c)), level_c);

        t8_debugf ("id of the computed nca: %li, level: %i\n",
                   static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t>(scheme_id), check, level_nca)), level_nca);
...
TEST_P (nca, recursive_check_higher_level)
{
  ...
  for (i = recursion_depth; i < max_lvl; i++) {
    leaves_on_level = scheme->element_count_leaves (static_cast<t8_eclass_t>(scheme_id), correct_nca, i - recursion_depth);
    /* middle = leaves/2 */
    scheme->element_set_linear_id (static_cast<t8_eclass_t>(scheme_id), correct_nca_high_level, i - recursion_depth, leaves_on_level / 2);
