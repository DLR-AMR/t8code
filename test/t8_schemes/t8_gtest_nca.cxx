...
class nca: public testing::TestWithParam<t8_eclass> {
 protected:
  ...
  SetUp () override
  {
    ...
    scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &correct_nca);
    scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &desc_a);
    scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &desc_b);
    scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &check);
    scheme->element_set_linear_id (static_cast<t8_eclass_t> (scheme_id), correct_nca, 0, 0);
  }
  void
  TearDown () override
  {
    scheme->element_destroy (static_cast<t8_eclass_t> (scheme_id), 1, &correct_nca);
    scheme->element_destroy (static_cast<t8_eclass_t> (scheme_id), 1, &desc_a);
    scheme->element_destroy (static_cast<t8_eclass_t> (scheme_id), 1, &desc_b);
...
{
  const int num_children = scheme->element_get_num_children (static_cast<t8_eclass_t> (scheme_id), correct_nca);
  /* Iterate over all combinations of two children from correct_nca */
  for (int i = 0; i < num_children - 1; i++) {
    scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), correct_nca, i, desc_a);
    for (int j = i + 1; j < num_children; j++) {
      scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), correct_nca, j, desc_b);
      /*Compute the nca */
...
TEST_P (nca, nca_check_deep)
{
  ...
  const int elem_max_level = scheme->get_maxlevel (static_cast<t8_eclass_t> (scheme_id));
  ...
  scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &tmp);
  scheme->element_copy (static_cast<t8_eclass_t> (scheme_id), correct_nca, tmp);
  for (lvl = 1; lvl <= max_lvl; lvl++) {
    num_children = scheme->element_get_num_children (static_cast<t8_eclass_t> (scheme_id), tmp);
    for (child_id = 0; child_id < num_children; child_id++) {
      scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), tmp, child_id, correct_nca);
      ...
      for (check_lvl_a = lvl + 1; check_lvl_a < elem_max_level; check_lvl_a++) {
        scheme->element_get_first_descendant (static_cast<t8_eclass_t> (scheme_id), correct_nca, desc_a, check_lvl_a);
        for (check_lvl_b = lvl + 1; check_lvl_b < elem_max_level; check_lvl_b++) {
          scheme->element_get_last_descendant (static_cast<t8_eclass_t> (scheme_id), correct_nca, desc_b, check_lvl_b);
          /* Compute the nca of desc_a and desc_b */
          scheme->element_get_nca (static_cast<t8_eclass_t> (scheme_id), desc_a, desc_b, check);
          if (tree_class == T8_ECLASS_VERTEX) {
            /* first- last-descendant logic does not hold for vertices. */
            EXPECT_EQ (scheme->element_get_level (static_cast<t8_eclass_t> (scheme_id), check),
                       SC_MIN (scheme->element_get_level (static_cast<t8_eclass_t> (scheme_id), desc_a),
                               scheme->element_get_level (static_cast<t8_eclass_t> (scheme_id), desc_b)));
          }
          else {
            /* Expect equality of correct_nca and check for every other class */
            EXPECT_ELEM_EQ (scheme, scheme_id, correct_nca, check);
          }
        }
      }
    }
    ...
    if (num_children != 1) {
      scheme->element_copy (static_cast<t8_eclass_t> (scheme_id), tmp, correct_nca);
      /* Continue in the middle */
      scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), correct_nca, num_children / 2, tmp);
    }
    else {
      scheme->element_copy (static_cast<t8_eclass_t> (scheme_id), correct_nca, tmp);
    }
  }
  scheme->element_destroy (static_cast<t8_eclass_t> (scheme_id), 1, &tmp);
}
                        ...
                        const t8_eclass_t tree_class)
{
  ...
  int level_a = scheme->element_get_level (static_cast<t8_eclass_t> (scheme_id), parent_a);
  int level_b = scheme->element_get_level (static_cast<t8_eclass_t> (scheme_id), parent_b);
  ...
  num_children_a = scheme->element_get_num_children (static_cast<t8_eclass_t> (scheme_id), parent_a);
  num_children_b = scheme->element_get_num_children (static_cast<t8_eclass_t> (scheme_id), parent_b);

  /* Iterate over all children of parent_a */
  for (i = 0; i < num_children_a; i++) {
    scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), parent_a, i, desc_a);
    /* Iterate over all children of parent_b */
    for (j = 0; j < num_children_b; j++) {
      scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), parent_b, j, desc_b);
      scheme->element_get_nca (static_cast<t8_eclass_t> (scheme_id), desc_a, desc_b, check);

      if (!scheme->element_is_equal (static_cast<t8_eclass_t> (scheme_id), check_nca, check)) {
        level_a = scheme->element_get_level (static_cast<t8_eclass_t> (scheme_id), desc_a);
        level_b = scheme->element_get_level (static_cast<t8_eclass_t> (scheme_id), desc_b);

        int level_c = scheme->element_get_level (static_cast<t8_eclass_t> (scheme_id), check_nca);
        int level_nca = scheme->element_get_level (static_cast<t8_eclass_t> (scheme_id), check);
        ...
        t8_debugf (
          "id of desc_a: %li, level: %i\n",
          static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t> (scheme_id), desc_a, level_a)),
          level_a);
        t8_debugf (
          "id of desc_b: %li, level: %i\n",
          static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t> (scheme_id), desc_b, level_b)),
          level_b);

        for (int k = SC_MAX (level_a, level_b); k >= 0; k--) {
          t8_debugf (
            "id of desc_a: %li, level: %i\n",
            static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t> (scheme_id), desc_a, k)), k);
          t8_debugf (
            "id of desc_b: %li, level: %i\n",
            static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t> (scheme_id), desc_b, k)), k);
        }

        t8_debugf (
          "id of the correct nca: %li, level: %i\n",
          static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t> (scheme_id), check_nca, level_c)),
          level_c);

        t8_debugf (
          "id of the computed nca: %li, level: %i\n",
          static_cast<long> (scheme->element_get_linear_id (static_cast<t8_eclass_t> (scheme_id), check, level_nca)),
          level_nca);
    ...
    scheme->element_get_parent (static_cast<t8_eclass_t> (scheme_id), desc_a, parent_a);
  }
}
...
TEST_P (nca, recursive_check)
{
  ...
  int num_children;
  num_children = scheme->element_get_num_children (static_cast<t8_eclass_t> (scheme_id), correct_nca);
  int i, j;
  if (num_children > 1) {
    scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &parent_a);
    scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &parent_b);
    scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), correct_nca, 0, parent_a);
    scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), correct_nca, 1, parent_b);
    for (i = 0; i < num_children - 1; i++) {
      scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), correct_nca, i, parent_a);
  ...
  scheme->element_destroy (static_cast<t8_eclass_t> (scheme_id), 1, &parent_a);
  scheme->element_destroy (static_cast<t8_eclass_t> (scheme_id), 1, &parent_b);
}
...
TEST_P (nca, recursive_check_higher_level)
{
  ...
  const int recursion_depth = 3;
  const int max_lvl = scheme->get_maxlevel (static_cast<t8_eclass_t> (scheme_id));
  ...
  scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &parent_a);
  scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &parent_b);
  scheme->element_new (static_cast<t8_eclass_t> (scheme_id), 1, &correct_nca_high_level);

  /* Test on different levels around the middle of the refinement tree */
  for (i = recursion_depth; i < max_lvl; i++) {
    leaves_on_level
      = scheme->element_count_leaves (static_cast<t8_eclass_t> (scheme_id), correct_nca, i - recursion_depth);
    /* middle = leaves/2 */
    scheme->element_set_linear_id (static_cast<t8_eclass_t> (scheme_id), correct_nca_high_level, i - recursion_depth,
                                   leaves_on_level / 2);

    /* Initialization for recursive_nca_check */
    num_children = scheme->element_get_num_children (static_cast<t8_eclass_t> (scheme_id), correct_nca_high_level);
    if (num_children > 1) {
      ...
      for (k = 0; k < num_children; k++) {
        scheme->element_get_child (static_cast<t8_eclass_t> (scheme_id), correct_nca_high_level, k, parent_a);
        for (l = 0; l < num_children; l++) {
          ...
          if (k != l) {
            ...
            EXPECT_ELEM_EQ (scheme, scheme_id, parent_a, check);
            EXPECT_ELEM_EQ (scheme, scheme_id, parent_b, check);
          }
        }
      }
    }
