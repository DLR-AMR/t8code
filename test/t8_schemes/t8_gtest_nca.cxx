  ...
  for (i = 0; i < num_children_a; i++) {
    ...
    for (j = 0; j < num_children_b; j++) {
      ...
      if (!ts->t8_element_equal (check_nca, check)) {
        ...
        t8_debugf ("id of desc_a: %li, level: %i\n", static_cast< long >(ts->t8_element_get_linear_id (desc_a, level_a)), level_a);
        t8_debugf ("id of desc_b: %li, level: %i\n", static_cast< long >(ts->t8_element_get_linear_id (desc_b, level_b)), level_b);
        ...
        t8_debugf ("id of the correct nca: %li, level: %i\n", static_cast< long >(ts->t8_element_get_linear_id (check_nca, level_c)),
                   level_c);

        t8_debugf ("id of the computed nca: %li, level: %i\n", static_cast< long >(ts->t8_element_get_linear_id (check, level_nca)),
                   level_nca);
