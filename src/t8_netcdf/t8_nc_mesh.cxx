...
static
void t8_nc_validate_initial_refinement_level_for_dimensionality(const int initial_refinement_level, const int dimensionality)
{ 
    ...
    {
        t8_errorf("The corresponding refinement level is not within the range of an computationally posible refinement level.");
    }
...
public:
    ...
    t8_nc_adapt_data_initial_mesh(const t8_nc_geo_domain_t& domain, const int initial_refinement_lvl, const t8_nc_data_layout_t layout)
...
bool
t8_nc_is_mesh_element_in_geo_domain(const t8_element_t* element, t8_eclass_scheme_c* ts, const t8_nc_geo_domain_t& reference_domain, const int initial_refinement_level, const t8_nc_data_layout_t initial_layout)
{
    ...
    if (dimensionality == 2)
    {
        ...
        switch (initial_layout)
        {
            ...
            case t8_nc_data_layout_t::LON_LAT:    
                if (element_anchor[0] >= reference_domain.get_dimension_start_index(t8_nc_dimension_t::LON) && element_anchor[0] < reference_domain.get_dimension_end_index(t8_nc_dimension_t::LON) &&
                    element_anchor[1] >= reference_domain.get_dimension_start_index(t8_nc_dimension_t::LAT) && element_anchor[1] < reference_domain.get_dimension_end_index(t8_nc_dimension_t::LAT))
                {
            ...
            case t8_nc_data_layout_t::LEV_LAT:
                if (element_anchor[0] >= reference_domain.get_dimension_start_index(t8_nc_dimension_t::LAT) && element_anchor[0] < reference_domain.get_dimension_end_index(t8_nc_dimension_t::LAT) &&
                    element_anchor[1] >= reference_domain.get_dimension_start_index(t8_nc_dimension_t::LEV) && element_anchor[1] < reference_domain.get_dimension_end_index(t8_nc_dimension_t::LEV))
                {
            ...
            break;
            default:
                t8_errorf("There was no valid 2D data layout supplied to determine whether or not the element is within the domain.");
        }
        ...
        if (element_anchor[0] >= reference_domain.get_dimension_start_index(t8_nc_dimension_t::LON) && element_anchor[0] < reference_domain.get_dimension_end_index(t8_nc_dimension_t::LON) &&
            element_anchor[1] >= reference_domain.get_dimension_start_index(t8_nc_dimension_t::LAT) && element_anchor[1] < reference_domain.get_dimension_end_index(t8_nc_dimension_t::LAT) &&
            element_anchor[2] >= reference_domain.get_dimension_start_index(t8_nc_dimension_t::LEV) && element_anchor[2] < reference_domain.get_dimension_end_index(t8_nc_dimension_t::LEV))
        {
...
t8_locidx_t
t8_nc_refine_to_initial_mesh (t8_forest_t forest,
                     ...
                     t8_element_t * elements[])
{
    ...
    t8_nc_adapt_data_initial_mesh* adapt_data = static_cast<t8_nc_adapt_data_initial_mesh*>(t8_forest_get_user_data(forest));
    ...
    if (t8_nc_is_mesh_element_in_geo_domain(elements[0], ts, adapt_data->global_domain, adapt_data->initial_refinement_level, adapt_data->initial_layout))
    {
        return 1;
    } else
    {
...
t8_nc_build_initial_embedded_mesh (const t8_nc_geo_domain_t& domain, const t8_nc_data_layout_t initial_layout,
    ...
    initial_forest = t8_forest_new_adapt(initial_forest, t8_nc_refine_to_initial_mesh, 1, 0, static_cast<void*>(&adapt_data));
...
t8_nc_build_initial_congruent_mesh(const t8_nc_geo_domain_t& domain, const t8_nc_data_layout_t initial_layout,
    ...
    const double dimension_length_x = static_cast<double>(domain.get_dimension_length (t8_nc_dimension_t::LON))/max_elem_per_direction;

    const double dimension_length_y = static_cast<double>(domain.get_dimension_length (t8_nc_dimension_t::LAT))/max_elem_per_direction;

    const double dimension_length_z = static_cast<double>(domain.get_dimension_length (t8_nc_dimension_t::LEV))/max_elem_per_direction;

    const double boundary_coords[24] = { 0, 0, 0, 
                                          dimension_length_x, 0, 0, 
                                          0, dimension_length_y, 0, 
                                          dimension_length_x, dimension_length_y, 0, 
                                          0, 0, dimension_length_z, 
                                          dimension_length_x, 0, dimension_length_z, 
                                          0, dimension_length_y, dimension_length_z, 
                                          dimension_length_x, dimension_length_y, dimension_length_z };

    t8_cmesh_t cmesh;
    auto [initial_refinement_level, number_of_trees] = t8_nc_calculate_initial_refinement_level_and_number_of_trees(domain);
    if (dimensionality == 2) {
      
      cmesh = t8_cmesh_new_hypercube_pad (t8_nc_dimension_to_element_class(dimensionality), comm, boundary_coords, number_of_trees[0], number_of_trees[1], 0, false);
    } else {

      cmesh = t8_cmesh_new_hypercube_pad (t8_nc_dimension_to_element_class(dimensionality), comm, boundary_coords, number_of_trees[0], number_of_trees[1], number_of_trees[2], false);
    }
    /* Construct a forest from the cmesh */
    t8_forest_t initial_forest = t8_forest_new_uniform(cmesh, t8_scheme_new_default_cxx(), initial_refinement_level, 0, comm);
