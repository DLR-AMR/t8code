 ...
 * Set stretch factors of the cover element to the maximum of the elements, 
 * that are covered by this cover element.
 * \param [in] forest               The forest.
 * \return vector of coverelement, sorted by lin id.
 */
void
// t8_forest_ghost_definition_overlap::
build_owe_cover (t8_forest_t forest)
{
    /**
     * 1.   Build a cover of the own process with the helper function.
     * This cover (stored as std::vector) does not have the right stretch factors.
     * 2.   To update the stretch factor iterate over all the own elements.
     *      Update the stretch factor to the maximum of the factors of his descendants.
     *      Use that the cover and the local elements are sorted.
     */
    /* 1. */
    ...
    int currend_cover_index = 0;
    /** Set the lin id of the next element in the cover, if this exists. */
    t8_linearidx_t next_cover_lin_id = 0;
    /** The last element of the cover is treated diffend, because there is no next_cover_lin_id */
    bool last_cover_element = true;
    if(cover.size() > (size_t) currend_cover_index + 1 ){
        next_cover_lin_id = t8_element_get_linear_id(eclass_scheme, tree_class, cover[currend_cover_index+1], max_level);
        last_cover_element = false;
    }
    
    /** Iterate over the elements of the forest. */
    for (t8_locidx_t ielement = 0; ielement < num_elements_in_tree; ++ielement){
        element = t8_forest_get_leaf_element(forest, ielement, 0);
        /** Check if the element is covered by the current cover element. */
        if(!last_cover_element && t8_element_get_linear_id(eclass_scheme, tree_class, element, max_level) >= next_cover_lin_id){
            /** The element is covert form the next element in the cover. Update the current cover index. */
            currend_cover_index++;
            next_cover_lin_id = t8_element_get_linear_id(eclass_scheme, tree_class, cover[currend_cover_index], max_level);
            /** Check if the new current cover element is the last one. */
            if(cover.size() <= (size_t) currend_cover_index + 1 ){
                last_cover_element = true;
            }
        }/** End check and update of current cover index. */
        /** Carst the element and the element of the cover to an standalone element, to get access to the stretch factor. */
        const t8_eclass_t tree_class = t8_forest_get_tree_class(forest, 0);
        if(tree_class == T8_ECLASS_QUAD){
            const t8_standalone_element<T8_ECLASS_QUAD> * element_carsted = (const t8_standalone_element<T8_ECLASS_QUAD> *) element;
            t8_standalone_element<T8_ECLASS_QUAD> * cover_element_carsted = ( t8_standalone_element<T8_ECLASS_QUAD> *) cover[currend_cover_index];
            for (int dim = 0; dim < 3; ++dim){
                /** Update the stretch factor of cover element, if the stretch factor of the element is high. */
                cover_element_carsted->stretch_factors[dim] = std::max(element_carsted->stretch_factors[dim],cover_element_carsted->stretch_factors[dim]);
            }
        }else if(tree_class == T8_ECLASS_HEX){
            const t8_standalone_element<T8_ECLASS_HEX> * element_carsted = (const t8_standalone_element<T8_ECLASS_HEX> *) element;
            t8_standalone_element<T8_ECLASS_HEX> * cover_element_carsted = ( t8_standalone_element<T8_ECLASS_HEX> *) cover[currend_cover_index];
            for (int dim = 0; dim < 3; ++dim){
                /** Update the stretch factor of cover element, if the stretch factor of the element is high. */
                cover_element_carsted->stretch_factors[dim] = std::max(element_carsted->stretch_factors[dim],cover_element_carsted->stretch_factors[dim]);
            }
        }
    }/** End iteration over the elements to set the stretch factors of the cover elements. */
    /* Return final cover. */
    // return cover;
 }

// void
// t8_forest_ghost_definition_overlap::communicate_covers (t8_forest_t forest)
// {
// Instat of communicate the covers, we communicate the maximum stretch factor of every process.
    // sc_MPI_Allreduce (&stretch, &max_glob_stretch, 2, sc_MPI_INT, sc_MPI_MAX, comm);
// }