...
t8_forest_ghost_definition_overlap::communicate_max_stretch_factor (t8_forest_t forest)
{
  ...
  for (t8_locidx_t ielement = 0; ielement < num_leaf_elements; ++ielement) {
    ...
    if (tree_class == T8_ECLASS_QUAD) {
      const t8_standalone_element<T8_ECLASS_QUAD> *element_carsted
        = (const t8_standalone_element<T8_ECLASS_QUAD> *) element;
      for (int dim = 0; dim < 3; ++dim) {
        /** Update the max stretch factor, if necessary. */
        max_local_stretch_factors[dim]
          = std::max (element_carsted->stretch_factors[dim], max_local_stretch_factors[dim]);
      }
    }
    else if (tree_class == T8_ECLASS_HEX) {
      const t8_standalone_element<T8_ECLASS_HEX> *element_carsted
        = (const t8_standalone_element<T8_ECLASS_HEX> *) element;
      for (int dim = 0; dim < 3; ++dim) {
        /** Update the max stretch factor, if necessary. */
        max_local_stretch_factors[dim]
          = std::max (element_carsted->stretch_factors[dim], max_local_stretch_factors[dim]);
...
t8_forest_ghost_definition_overlap::search_for_ghost_elements (t8_forest_t forest)
{
  ...
  for (int i = 0; i < 2 * T8_ECLASS_MAX_DIM; i++) {
    // i%3 = 0,1,2, 0,1,2
    ref_cords[i]
      = t8_element_corner_ref_coords[tree_class][(i / T8_ECLASS_MAX_DIM) * (num_of_corners - 1)][i % T8_ECLASS_MAX_DIM];
  }
  ...
  for (t8_locidx_t ielement = 0; ielement < num_leaf_elements; ++ielement) {
    ...
    if (_has_uniform_stretch_factor) {
      ...
      t8_forest_element_from_ref_coords_ext (forest, 0, element, ref_cords, 2, coords_out,
                                             _uniform_stretch_factor.data ());
    }
    else {
      ...
      if (tree_class == T8_ECLASS_QUAD) {
        ...
        t8_forest_element_from_ref_coords_ext (forest, 0, element, ref_cords, 2, coords_out,
                                               element_carsted->stretch_factors);
      }
      else if (tree_class == T8_ECLASS_HEX) {
    ...
    for (int remote_rank = 0; remote_rank < forest->mpisize; ++remote_rank) {
      ...
      for (int icover_element = 0; icover_element < (int) _list_of_covers[remote_rank].size (); ++icover_element) {
        ...
        t8_forest_element_from_ref_coords_ext (forest, 0, _list_of_covers[remote_rank][icover_element], ref_cords, 2,
                                               coords_cover_element, stretch_factor_cover_element);
 ...
 */
int
t8_ghost_puma_recursion_first_descandance (t8_forest_t forest, const t8_eclass_t tree_class,
                                           const t8_scheme *eclass_scheme, const t8_element_t *first_element,
                                           const t8_linearidx_t lin_id_first_element, const t8_element_t *ancestors,
    ...
    if (child_found) {
      ...
      if (eclass_scheme->element_is_equal (tree_class, children[child_index], child_first_nca)) {
        ...
        if (eclass_scheme->element_get_linear_id (tree_class, children[child_index], max_level)
          ...
          t8_ghost_puma_recursion_first_descandance (forest, tree_class, eclass_scheme, first_element,
                                                     lin_id_first_element, children[child_index], cover);
        }
      }
    }
  }
 ...
 */
int
t8_ghost_puma_recursion_last_descandance (t8_forest_t forest, const t8_eclass_t tree_class,
                                          const t8_scheme_c *eclass_scheme, const t8_element_t *last_element,
                                          const t8_linearidx_t lin_id_last_element, const t8_element_t *ancestors,
                                          std::vector<t8_element_t *> &cover)
{
  /* Allocate memory for the childrens. */
  t8_element_t **children = T8_ALLOC (t8_element_t *, 4);
 ...
 */
int
build_cover_forward_iteration (t8_forest_t forest, t8_element_t **children, const t8_eclass_t tree_class,
                               const t8_scheme_c *eclass_scheme, const t8_element_t *first_element,
                               const t8_linearidx_t lin_id_first_element, std::vector<t8_element_t *> &cover)
{
 ...
 */
int
build_cover_backward_iteration (t8_forest_t forest, t8_element_t **children, const t8_eclass_t tree_class,
                                const t8_scheme_c *eclass_scheme, const t8_element_t *last_element,
                                const t8_linearidx_t lin_id_last_element,
  ...
  for (int child_index = 3; child_index > -1; child_index--) {
    ...
    if (eclass_scheme->element_is_equal (tree_class, children[child_index], child_first_nca)) {
      ...
      if (eclass_scheme->element_get_linear_id (tree_class, children[child_index], max_level) != lin_id_last_element) {
        ...
        t8_ghost_puma_recursion_last_descandance (forest, tree_class, eclass_scheme, last_element, lin_id_last_element,
                                                  children[child_index], revers_last_cover_part);
      }
...
build_cover_of_process (t8_forest_t forest, const int process)
{
  ...
  const t8_linearidx_t lin_id_first_element
    = *(t8_linearidx_t *) t8_shmem_array_index (forest->global_first_desc, process);
  ...
  const t8_linearidx_t lin_id_last_element
    = (process + 1 < forest->mpisize)
        ? *(t8_linearidx_t *) t8_shmem_array_index (forest->global_first_desc, process + 1)
        : 0;
  ...
  /**
     ...
     * to fill the cover with the in between of the children witch war parent of first and last element. 
     */
  const int parent_of_first_element_and_child_of_nca = build_cover_forward_iteration (
    forest, children, tree_class, eclass_scheme, first_element, lin_id_first_element, cover);
  /**
  ...
  if (process + 1 < forest->mpisize) {
    parent_of_last_element_and_child_of_nca = build_cover_backward_iteration (
      forest, children, tree_class, eclass_scheme, last_element, lin_id_last_element, revers_last_cover_part);
  }
  /**
     * Cover of the child of nca that are in between 
     */
  for (int inbetween_childrens = parent_of_first_element_and_child_of_nca + 1;
       inbetween_childrens < parent_of_last_element_and_child_of_nca; ++inbetween_childrens) {
...
t8_forest_ghost_definition_overlap::build_all_cover (t8_forest_t forest)
{
  ...
  for (int p = 0; p < forest->mpisize; ++p) {
    ...
    if (_max_stretch_factors != NULL || _has_uniform_stretch_factor) {
      ...
      for (int dim = 0; dim < 3; ++dim) {
        max_stretch_factors[dim] = _has_uniform_stretch_factor
                                     ? _uniform_stretch_factor[dim]
                                     : *(double *) t8_shmem_array_index (_max_stretch_factors, 3 * p + dim);
      }
      ...
      for (int index = 0; index < (int) cover.size (); ++index) {
        if (tree_class == T8_ECLASS_QUAD) {
          t8_standalone_element<T8_ECLASS_QUAD> *element_carsted
            = (t8_standalone_element<T8_ECLASS_QUAD> *) cover[index];
          for (int dim = 0; dim < 3; ++dim) {
