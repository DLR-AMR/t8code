...
/** \file t8_forest_ghost_definition_helpers.cxx
...
#undef T8_POLLING /* activates polling for Mpi messages, currently used for testing */
    ...
    while (received_messages < num_remotes - 1) {
       ...
       *       blocking Probe. */
      iprobe_flag = 0;
      prev = nullptr; /* ensure that if the first receive entry is matched first,
                                it is removed properly. */
      for (proc_it = receivers->first; proc_it != nullptr && iprobe_flag == 0;) {
