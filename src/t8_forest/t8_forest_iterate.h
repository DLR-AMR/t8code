 ...
 * \param[in] element             the element for which the search criterion is checked
 * \param[in] pfirst              the lowest processor that owns part of \a element. Guaranteed to be non-empty.
 * \param[in] plast               the highest processor that owns part of \a element. Guaranteed to be non-empty.
 * \returns                       non-zero if the search criterion is met, zero otherwise.
 */
typedef int (*t8_forest_partition_search_fn) (t8_forest_t forest, const t8_locidx_t ltreeid,
 ...
 *                                \a query_matches and entries of \a query_indices).
 */
typedef int (*t8_forest_partition_query_fn) (t8_forest_t forest, const t8_locidx_t ltreeid, const t8_element_t *element,
                                             const int pfirst, const int plast, void *queries,
