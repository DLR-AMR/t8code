...
{
  constexpr size_t input_dim = t8_dim_v<typename TRefCoords::value_type>;
  T8_ASSERTF (std::ranges::size (ref_coords) <= std::ranges::size (coords_out),
              "coords_out is smaller than ref_coords.");

  const t8_eclass_t tree_class = t8_forest_get_tree_class (forest, ltreeid);
  const size_t tree_dim = (size_t) t8_eclass_to_dimension[tree_class];
  const size_t coord_dim = (tree_dim == 0 ? 1 : tree_dim);

  T8_ASSERTF (input_dim >= coord_dim, "Input reference coordinates do not have the right dimension.");
  ...
  std::vector<t8_point<input_dim>> tree_ref_coords (std::size (ref_coords));
  ...
  if (stretch_factors != NULL) {
    ...
    std::vector<t8_point<input_dim>> stretched_ref_coords;
    stretched_ref_coords.reserve (std::size (ref_coords));
    for (size_t ipoint = 0; ipoint < std::size (ref_coords); ++ipoint) {
      t8_point<input_dim> stretched_point;
      for (size_t idim = 0; idim < input_dim; ++idim) {
        stretched_point[idim] = 0.5 + ((ref_coords[ipoint][idim] - 0.5) * stretch_factors[idim]);
      }
      stretched_ref_coords.push_back (std::move (stretched_point));
    }
  ...
  converted_ref_coords.reserve (coord_dim * std::ranges::size (ref_coords));
  for (const auto &coord : tree_ref_coords) {
    for (size_t dim = 0; dim < coord_dim; ++dim)
      converted_ref_coords.push_back (coord[dim]);
  }
  T8_ASSERT (converted_ref_coords.size () == std::ranges::size (ref_coords) * coord_dim);
  double *temp_coords = T8_ALLOC (double, std::size (ref_coords) * 3);
  t8_geometry_evaluate (cmesh, gtreeid, converted_ref_coords.data (), std::ranges::size (ref_coords), temp_coords);
  for (size_t icoord = 0; icoord < std::ranges::size (ref_coords); ++icoord) {
    for (size_t dim = 0; dim < 3; ++dim) {
      coords_out[icoord][dim] = temp_coords[icoord * 3 + dim];
    }
  }
  T8_FREE (temp_coords);
  /* End of conversion */
