...
*num_conforming, t8_locidx_t *num_nonconforming, t8_locidx_t *num_boundaries)
{
  ...
  for (t8_locidx_t itree = 0, current_index = 0; itree < num_local_trees; ++itree) {
    ...
    for (t8_locidx_t ielement = 0; ielement < num_elements_in_tree; ++ielement, ++current_index) {
      ...
      for (int iface = 0; iface < num_faces; iface++) {
        ...
        if (num_neighbors > 0) {
          int neighbor_level = t8_element_level(neigh_scheme, neighbors[0]);
  ...
  *num_nonconforming = local_num_mortars;
...
t8_forest_get_local_num_interfaces_double (t8_forest_t forest)
{
  ...
  t8_locidx_t local_num_interfaces = 0;
  ...
  for (t8_locidx_t itree = 0, current_index = 0; itree < num_local_trees; ++itree) {
    ...
    for (t8_locidx_t ielement = 0; ielement < num_elements_in_tree; ++ielement, ++current_index) {
      ...
      for (int iface = 0; iface < num_faces; iface++) {
        ...
        local_num_interfaces = local_num_interfaces + (num_neighbors > 0 ? num_neighbors : 1);
  ...
  return local_num_interfaces;
}

void
t8_forest_get_local_element_levels (t8_forest_t forest, int *levels) {
...
static int
get_opposite_face_child_id(t8_forest_t forest, t8_locidx_t itree, t8_element_t *element, int iface, int opposite_elementid)
{
  int                 num_neighbors;
  int                *dual_faces;
  t8_locidx_t        *neighids;
  ...
  int opposite_face_child_id = -1;
 
  for (int ineigh = 0; ineigh < num_neighbors; ineigh++) {
    if (opposite_elementid == neighids[ineigh]) {
      opposite_face_child_id = ineigh;
      break;
    }
  }
    ...
    for (t8_locidx_t ielement = 0; ielement < num_elements_in_tree; ++ielement, ++current_index) {
      ...
      for (int iface = 0; iface < num_faces; iface++) {
        ...
        if (t8_element_is_root_boundary(eclass_scheme, element, iface) == 1) {
          int iface_in_tree = t8_element_tree_face(eclass_scheme, element, iface);
          t8_cmesh_get_face_neighbor(cmesh, itree_in_cmesh, iface_in_tree, NULL, &orient);
        }

        int                 num_neighbors;
        int                *dual_faces;
        t8_locidx_t        *neighids;
        t8_element_t      **neighbors_;
        t8_eclass_scheme_c *neigh_scheme;
        ...
        if (num_neighbors > 0) {
          int neighbor_level = t8_element_level(neigh_scheme, neighbors_[0]);

          for (int ineigh = 0; ineigh < num_neighbors; ineigh++) {
            orientation[local_faceid + ineigh] = orient;

            primary_id[local_faceid + ineigh] = current_index;
            secondary_id[local_faceid + ineigh] = neighids[ineigh];

            primary_face[local_faceid + ineigh] = iface;
            secondary_face[local_faceid + ineigh] = dual_faces[ineigh];

            if (level <= neighbor_level) {
              primary_child_id[local_faceid + ineigh] = 0;
              secondary_child_id[local_faceid + ineigh] = ineigh;
            } else {
              primary_child_id[local_faceid + ineigh] = 
                get_opposite_face_child_id(forest, itree, neighbors_[ineigh], dual_faces[ineigh], current_index);
              secondary_child_id[local_faceid + ineigh] = 0;
            }
          }

          local_faceid = local_faceid + num_neighbors;
        } else {
          orientation[local_faceid] = orient;

          primary_id[local_faceid] = current_index;
          secondary_id[local_faceid] = -1;

          primary_face[local_faceid] = iface;
          secondary_face[local_faceid] = -1;

          local_faceid = local_faceid + 1;

          primary_child_id[local_faceid] = 0;
          secondary_child_id[local_faceid] = -1;
        }
