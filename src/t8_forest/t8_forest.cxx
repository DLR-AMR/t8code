                              ...
                              double *coordinates)
{
  ...
  t8_geometry_evaluate (cmesh, gtreeid, vertex_coords.data (), 1, coordinates);
}
                                   ...
                                   const double *ref_coords, const size_t num_coords, double *coords_out)
{
  ...
  const int ref_coords_dim = t8_eclass_to_dimension[tree_class] + t8_eclass_to_dimension[tree_class] == 0 ? 1 : 0;

  std::vector<std::span<double>> ref_coords_view;
  ref_coords_view.reserve (num_coords);
  for (std::size_t coord = 0; coord < num_coords; ++coord)
    ref_coords_view.emplace_back (ref_coords + coord * ref_coords_dim, ref_coords_dim);

  std::vector<std::span<double>> out_coords_view;
  out_coords_view.reserve (num_coords);
  for (std::size_t coord = 0; coord < num_coords; ++coord)
    ref_coords_view.emplace_back (coords_out + coord * T8_ECLASS_MAX_DIM, T8_ECLASS_MAX_DIM);

  t8_forest_element_from_ref_coords_ext (forest, ltreeid, element, ref_coords_view, out_coords_view, NULL);
}
