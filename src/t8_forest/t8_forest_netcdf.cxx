                                  ...
                                  namespace_context)
{
  ...
  if ((retval =
       nc_put_att_int (context->ncid, context->var_elem_types_id,
                       "_FillValue", NC_INT, 1, &context->fillvalue32))) {
    ERR (retval);
  }
  ...
  /* Define _FillValue attribute */
  if ((retval =
       nc_put_att_long (context->ncid, context->var_elem_tree_id,
                        "_FillValue", NC_INT64, 1, &context->fillvalue64))) {
    ERR (retval);
  }
  /* Define start_index attribute. */
  if ((retval =
  ...
  /* Define _FillValue attribute */
  if ((retval =
       nc_put_att_long (context->ncid, context->var_elem_nodes_id,
                        "_FillValue", NC_INT64, 1, &context->fillvalue64))) {
    ERR (retval);
  }
  /* Define start_index attribute. */
                             ...
                             sc_MPI_Comm comm)
{
  ...
  if ((retval =
       nc_put_vara_long (context->ncid, context->var_elem_tree_id, &start_ptr,
  ...
  if (num_extern_netcdf_vars > 0 && ext_variables != NULL) {
    ...
    for (i = 0; i < num_extern_netcdf_vars; i++) {
      ...
      if ((retval =
           nc_put_att_text (context->ncid, (ext_variables[i]->var_user_dimid),
                            "units",
                            strlen (ext_variables[i]->variable_units),
                            ext_variables[i]->variable_units))) {
        ERR (retval);
      }
    }
  }
                                        ...
                                        sc_MPI_Comm comm)
{
  ...
  Mesh_elem_nodes =
    T8_ALLOC (t8_nc_int64_t, num_elements * num_max_nodes_per_elem);
  ...
  if ((retval =
       nc_put_vara_long (context->ncid, context->var_elem_nodes_id,
                             ...
                             sc_MPI_Comm comm)
{
...
#if T8_WITH_NETCDF_PAR
    if ((retval =
         nc_create_par (context->filename, cmode, comm, sc_MPI_INFO_NULL,
                        &context->ncid))) {
      ERR (retval);
    }
    t8_debugf ("A parallel netCDf-file has been created.\n");
#elif T8_WITH_NETCDF
    if ((retval = nc_create (context->filename, cmode, &context->ncid))) {
      ERR (retval);
    }
  ...
  if (context->multifile_mode) {
    ...
    if ((retval =
         nc_put_att_int (context->ncid, NC_GLOBAL, "part_index", NC_INT, 1,
                         &mpirank))) {
      ERR (retval);
    }
    /* Define part_count attribute */
    if ((retval =
         nc_put_att_int (context->ncid, NC_GLOBAL, "part_count", NC_INT, 1,
                         &mpisize))) {
      ERR (retval);
    }
  }
                            ...
                            bool multifile_mode)
{
  ...
  if (fill_mode != NC_FILL && fill_mode != NC_NOFILL) {
    t8_global_productionf
      ("Illegal fill-mode (must be NC_FILL or NC_NOFILL). Using NC_FILL.");
    fill_mode = NC_FILL;
  }
                        ...
                        sc_MPI_Comm comm)
{
  ...
  t8_forest_write_netcdf_ext (forest, file_prefix, file_title, dim,
                              num_extern_netcdf_vars, ext_variables, comm,
                              netcdf_var_storage_mode, nullptr,
                              netcdf_mpi_access, NC_NOFILL, NC_NETCDF4,
                              false);
}
