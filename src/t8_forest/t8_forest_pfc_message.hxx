...
class t8_forest_pfc_message_c {
 public:
  ...
  fill (t8_forest_t forest)
  {
    ...
    t8_forest_pfc_helper_index_in_tree_from_globalid (forest, closest_to_rank_gid, itree, scheme, tree, index_in_tree,
    ...
    if (scheme->t8_element_level (element_closest_to_receiver) == 0) {
      ...
      if (iproc > rank) {
        t8_locidx_t min_id = t8_forest_pfc_extreme_local_sibling (scheme, tree, index_in_tree, true);
        num_siblings = index_in_tree - min_id + 1;
      }
      else {
        T8_ASSERT (iproc != rank);
        t8_locidx_t max_id = t8_forest_pfc_extreme_local_sibling (scheme, tree, index_in_tree, false);
  ...
  t8_forest_pfc_message_c (t8_scheme_cxx_t *schemes, t8_procidx_t iproc, sc_MPI_Comm comm)
    : itree (0), eclass (T8_ECLASS_ZERO), num_siblings (0), schemes (schemes), comm (comm), iproc (iproc),
      message_tag (T8_PFC_MESSAGE), parent (NULL), allocated_parent (0)
  {
    t8_scheme_cxx_ref (schemes);
  }
  t8_forest_pfc_message_c (const t8_forest_pfc_message_c &other) = delete;
  t8_forest_pfc_message_c (t8_forest_pfc_message_c &&other)
    : itree { other.itree }, eclass (other.eclass), num_siblings (other.num_siblings), schemes (other.schemes),
