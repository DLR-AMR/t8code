    ...
    if (t8_eclass_to_dimension[eclass] != dim) {
      ...
      for (tree_loop = 0; tree_loop < num_ele_in_block; tree_loop++, tree_count++) {
        ...
        if (!use_occ_geometry) {
          ...
          if (tree_is_linked) {
            t8_cmesh_set_tree_geometry (cmesh, tree_count, occ_geometry_base);
            t8_debugf ("Registering tree %li with geometry %s \n", tree_count,
                       occ_geometry_base->t8_geom_get_name ().c_str ());
          }
          else {
            t8_cmesh_set_tree_geometry (cmesh, tree_count, linear_geometry_base);
            t8_debugf ("Registering tree %li with geometry %s \n", tree_count,
                                            ...
                                            const t8_geometry_c **occ_geometry)
{
  ...
  *linear_geometry = t8_cmesh_register_geometry<t8_geometry_linear> (cmesh, dim);
  ...
  if (use_occ_geometry) {
#if T8_WITH_OCC
    *occ_geometry = t8_cmesh_register_geometry<t8_geometry_occ> (cmesh, dim, std::string (fileprefix));
#else /* !T8_WITH_OCC */
    *occ_geometry = NULL;
    return 0;
#endif
