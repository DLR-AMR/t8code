                             ...
                             int my_buffer_bytes)
{
  ...
  for (iproc = 0; iproc < num_receive; iproc++) {
    ...
    mpiret =
      sc_MPI_Probe (sc_MPI_ANY_SOURCE, T8_MPI_PARTITION_CMESH, cmesh->mpicomm,
                    status + iproc);
    SC_CHECK_MPI (mpiret);
    ...
    if (recv_first <= proc_recv && proc_recv <= recv_last &&
