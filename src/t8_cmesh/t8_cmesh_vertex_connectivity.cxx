                                      ...
                                      const t8_gloidx_t *global_tree_vertices, const int num_vertices)
{
  ...
  cmesh->vertex_connectivity->tree_to_vertex.set_global_vertex_ids_of_tree_vertices (cmesh, global_tree, global_tree_vertices, num_vertices);
}                                    
...
{
  T8_ASSERT (t8_cmesh_is_committed (cmesh));
  return cmesh->vertex_connectivity.tree_to_vertex.get_global_vertices (cmesh, local_tree, num_vertices);
}
...
t8_cmesh_get_vertex_to_tree_list (const t8_cmesh_t cmesh, const t8_gloidx_t global_vertex)
{
  ...
  return cmesh->vertex_connectivity.vtt.get_tree_list_of_vertex (global_vertex);
}
...
t8_cmesh_get_num_trees_at_vertex (const t8_cmesh_t cmesh, t8_gloidx_t global_vertex)
{
  ...
  return cmesh->vertex_connectivity.vtt.get_tree_list_of_vertex().length();
}
