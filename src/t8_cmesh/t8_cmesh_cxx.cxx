                                ...
                                int8_t * first_tree_shared, sc_MPI_Comm comm)
{
  ...
  t8_eclass_scheme_c *tree_scheme;
  T8_ASSERT (cmesh != NULL);
  const int           first_tree_shared_shift =
    cmesh->first_tree_shared ? 1 : 0;
#ifdef T8_ENABLE_DEBUG
  ...
  if (t8_cmesh_is_empty (cmesh)) {
    t8_cmesh_uniform_set_return_parameters_to_empty (first_local_tree,
                                                     child_in_tree_begin,
                                                     last_local_tree,
                                                     child_in_tree_end,
                                                     first_tree_shared);
    ...
    for (igtree = 0; igtree < num_trees; ++igtree) {
      ...
      if (current_tree_element_offset <= first_child &&
        ...
        *first_local_tree = igtree;
        ...
        if (first_tree_shared != NULL) {
          *first_tree_shared =
            current_tree_element_offset < first_child ? 1 : 0;
        }
      }
      ...
      if (current_tree_element_offset <= last_child &&
          last_child < current_tree_element_offset + elem_in_tree) {
        if (child_in_tree_end != NULL) {
  ...
  t8_shmem_array_init (&offset_array, sizeof (t8_gloidx_t),
  ...
  t8_shmem_prefix (&local_num_children, offset_array, 1, T8_MPI_GLOIDX,
                   sc_MPI_SUM);

  data.num_procs = cmesh->mpisize;
  /* Get global number of elements */
  data.global_num_elements =
    t8_shmem_array_get_gloidx (offset_array, cmesh->mpisize);
  SC_CHECK_ABORTF (0 <= data.global_num_elements
                   && data.global_num_elements < T8_GLOIDX_MAX,
  ...
  if (pure_local_trees > 0) {
    ...
    elem_index_pointer =
      (t8_gloidx_t *) sc_array_index_int (&first_element_tree, 0);
    *elem_index_pointer =
    ...
    for (itree = 0; itree < pure_local_trees; ++itree, ++igtree) {
      ...
      *first_element_of_next_tree =
        *first_element_of_tree +
        tree_scheme->t8_element_count_leafs_from_root (level);
    }

    /* Check that the last tree + 1 stores as offset the start of the next process */
    T8_ASSERT (*((t8_gloidx_t *)
                 t8_sc_array_index_locidx (&first_element_tree,
                                           pure_local_trees)) ==
    ...
    send_first_nonempty =
      (t8_gloidx_t) t8_cmesh_determine_partition (&first_element_tree, 0,
                                                  &data);
    ...
    send_last =
      (t8_gloidx_t) t8_cmesh_determine_partition (&first_element_tree,
                                                  pure_local_trees, &data);
    ...
    sc_array_split (&first_element_tree, &offset_partition,
                    (size_t) num_procs_we_send_to + 1,
    ...
    for (iproc = send_first; iproc <= send_last; iproc++) {
      ...
      const t8_gloidx_t   last_element_index_of_current_proc =
        t8_cmesh_get_first_element_of_process (iproc + 1, data.num_procs,
                                               data.global_num_elements) - 1;
      const int           proc_is_empty =
      ...
      if (!proc_is_empty) {
        ...
        const t8_locidx_t   possibly_first_puretree_of_current_proc =
          *((size_t *)
            sc_array_index_int (&offset_partition, iproc - send_first));
        const t8_locidx_t   possibly_first_puretree_of_next_proc =
          *((size_t *)
            sc_array_index_int (&offset_partition, iproc + 1 - send_first));
        const t8_gloidx_t   first_el_index_of_first_tree =
          *(t8_gloidx_t *) t8_sc_array_index_gloidx (&first_element_tree,
                                                     possibly_first_puretree_of_current_proc);

        if (first_element_index_of_current_proc >=
            last_el_index_of_last_tree + 1) {
        ...
        if (send_start_message) {
          ...
          if (first_el_index_of_first_tree >
              first_element_index_of_current_proc) {
            ...
            if (possibly_first_puretree_of_current_proc - 1 < 0) {
              ...
              first_puretree_of_current_proc =
                possibly_first_puretree_of_current_proc - 1;
            }
          }
          else {
            first_puretree_of_current_proc =
        ...
        if (send_end_message) {
          if (last_el_index_of_last_tree < last_element_index_of_current_proc) {
            ...
            if (iproc == send_last) {
            ...
            else if (possibly_first_puretree_of_next_proc ==
                     possibly_first_puretree_of_current_proc) {
              ...
              last_puretree_of_current_proc =
                possibly_first_puretree_of_next_proc - 1;
            }
          }
        }
        ...
        if (send_start_message) {
          T8_ASSERT (0 <= first_puretree_of_current_proc
                     && first_puretree_of_current_proc < pure_local_trees);
        }
        else {
        ...
        if (send_end_message) {
          T8_ASSERT (0 <= last_puretree_of_current_proc
                     && last_puretree_of_current_proc < pure_local_trees);
        }
        else {
          T8_ASSERT (last_puretree_of_current_proc == -1);
        }
        if (first_puretree_of_current_proc != -1
            && last_puretree_of_current_proc != -1) {
          T8_ASSERT (first_puretree_of_current_proc <=
                     last_puretree_of_current_proc);
        }
#endif
        if (send_start_message) {
          ...
          first_element_in_tree_index_of_current_proc =
            first_element_index_of_current_proc - first_el_of_first_tree;
        }
        if (send_end_message) {
          /* Compute the index inside the tree of the last element. */
          const t8_gloidx_t   first_el_of_last_tree = *(t8_gloidx_t *)
            t8_sc_array_index_locidx (&first_element_tree,
                                      last_puretree_of_current_proc);
          last_element_in_tree_index_of_current_proc =
            last_element_index_of_current_proc - first_el_of_last_tree;
          t8_debugf ("[H] Computing last element as %li - %li = %li\n\n",
                     last_element_index_of_current_proc,
                     first_el_of_last_tree,
                     last_element_in_tree_index_of_current_proc);
          t8_debugf ("[H] Last pure tree = %i, first element in it %li\n",
                     last_puretree_of_current_proc, first_el_of_last_tree);
        }
      }
      ...
      if (send_start_message) {
        const t8_gloidx_t   global_id_of_first_tree =
          proc_is_empty ? -1 :
          first_puretree_of_current_proc + t8_cmesh_get_first_treeid (cmesh) +
          first_tree_shared_shift;
        ...
        if (iproc != cmesh->mpirank) {
          ...
          sc_MPI_Request     *request =
            (sc_MPI_Request *) sc_array_push (&send_requests);

          /* Grow total send buffer */
          sc_array_push_count (&send_buffer, num_entries);
          /* Set message to current position in the buffer. */
          message =
            (t8_gloidx_t *) sc_array_index_int (&send_buffer,
                                                current_pos_in_send_buffer);
          ...
          t8_debugf ("[H]  first e index %li, first in tree %li\n",
                     first_element_index_of_current_proc,
                     first_puretree_of_current_proc >= 0 ? *(t8_gloidx_t *)
                     t8_sc_array_index_gloidx (&first_element_tree,
                                               first_puretree_of_current_proc)
                     : -1);
          mpiret =
            sc_MPI_Isend (message, num_entries, T8_MPI_GLOIDX, iproc,
                          T8_MPI_CMESH_UNIFORM_BOUNDS_START, comm, request);
          SC_CHECK_MPI (mpiret);
          t8_debugf
            ("Sending start message (%li, %li) to %i (global num el %li)\n",
             message[0], message[1], iproc, data.global_num_elements);

          T8_ASSERT (proc_is_empty || (0 <= message[0]
                                       && message[0] <
                                       t8_cmesh_get_num_trees (cmesh)));
          T8_ASSERT (proc_is_empty || (0 <= message[1]
                                       && message[1] <
                                       data.global_num_elements));
      ...
      if (send_end_message) {
        const t8_gloidx_t   global_id_of_last_tree =
          proc_is_empty ? -1 :
          last_puretree_of_current_proc + t8_cmesh_get_first_treeid (cmesh) +
          first_tree_shared_shift;
        if (iproc != cmesh->mpirank) {
          const int           num_entries = 2;
          t8_gloidx_t        *message;
          ...
          mpiret =
            sc_MPI_Isend (message, num_entries, T8_MPI_GLOIDX, iproc,
                          T8_MPI_CMESH_UNIFORM_BOUNDS_END, comm, request);
          SC_CHECK_MPI (mpiret);
  ...
  if (expect_start_message) {
    ...
    mpiret =
      sc_MPI_Recv (message, num_entries, T8_MPI_GLOIDX, sc_MPI_ANY_SOURCE,
                   T8_MPI_CMESH_UNIFORM_BOUNDS_START, comm,
    ...
    *first_local_tree = message[0];
    T8_ASSERT (*first_local_tree == -1 || (0 <= *first_local_tree
                                           && *first_local_tree <
                                           t8_cmesh_get_num_trees (cmesh)));
    if (message[1] > 0) {
      /* The first tree is shared */
    ...
    if (child_in_tree_begin != NULL) {
      *child_in_tree_begin = message[1];
      ...
      T8_ASSERT (*child_in_tree_begin == -1 || (0 <= *child_in_tree_begin
                                                && *child_in_tree_begin <
                                                data.global_num_elements));
    }
    T8_FREE (message);
  ...
  if (expect_end_message) {
    ...
    mpiret =
      sc_MPI_Recv (message, num_entries, T8_MPI_GLOIDX, sc_MPI_ANY_SOURCE,
                   T8_MPI_CMESH_UNIFORM_BOUNDS_END, comm,
                   sc_MPI_STATUS_IGNORE);
    SC_CHECK_MPI (mpiret);
#ifdef T8_ENABLE_DEBUG
    num_received_end_messages++;
    ...
    *last_local_tree = message[0];
    T8_ASSERT (*last_local_tree == -1 || (0 <= *last_local_tree
                                          && *last_local_tree <=
                                          t8_cmesh_get_num_trees (cmesh)));
    if (child_in_tree_end != NULL) {
      *child_in_tree_end = message[1] + 1;
      T8_ASSERT (*child_in_tree_end == -1 || (0 <= *child_in_tree_end
                                              && *child_in_tree_end <=
                                              data.global_num_elements));
    }
    T8_FREE (message);
  ...
  if (child_in_tree_begin != NULL && child_in_tree_end != NULL) {
    ...
    if (*child_in_tree_end < 0 || *child_in_tree_begin < 0) {
      ...
      t8_debugf ("[H] EMPTY hybrid %li %li %li %li\n", *first_local_tree,
                 *last_local_tree, *child_in_tree_begin, *child_in_tree_end);
  ...
  mpiret =
    sc_MPI_Waitall (num_messages_sent, (sc_MPI_Request *) send_requests.array,
