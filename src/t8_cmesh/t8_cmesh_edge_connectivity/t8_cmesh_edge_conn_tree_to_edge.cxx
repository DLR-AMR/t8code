...
/** \file t8_cmesh_edge_conn_tree_to_edge.cxx
 *  This file implements the routines for the t8_cmesh_edge_conn_tree_to_edge struct.
 */
...
/* constructor from a given edge to tree list. */
t8_cmesh_edge_conn_tree_to_edge::t8_cmesh_edge_conn_tree_to_edge (
  const t8_cmesh_t cmesh_from, const t8_cmesh_t cmesh, const struct t8_cmesh_edge_conn_edge_to_tree &ett)
  : t8_cmesh_edge_conn_tree_to_edge ()
{
  T8_ASSERT (t8_cmesh_is_committed (cmesh_from));
  T8_ASSERT (t8_cmesh_is_initialized (cmesh));
  T8_ASSERT (ett.is_committed ());
  ...
  /* Stores an array of local indices and mapped global edges for each tree.
  ...
  /* Iterate over all entries of the edge to tree list. */
  for (auto &[global_edge, edge_list] : ett) {
    /* Iterate over all entries of the edge list,
     * thus, each entry is a local tree id and edge index. */
    for (auto &[tree_index, edge_index] : edge_list) {
      global_local_index_pair new_pair (global_edge, edge_index);
  ...
  for (t8_locidx_t itree = 0; itree < num_local_trees_and_ghosts; ++itree) {
    ...
    /* Size of list must match number of tree edges. */
    SC_CHECK_ABORTF (
      tree_indices.size () == (size_t) num_tree_edges,
      "ERROR. Number of mapped local tree edges for tree %i does not equal number of tree edges. %zu != %i\n",
      itree, tree_indices.size (), num_tree_edges);
    /* We now build the array of global indices of this tree, sorted 
     * by the trees local indices. */
    t8_gloidx_t *global_tree_indices = T8_ALLOC (t8_gloidx_t, num_tree_edges);
    for (int iedge = 0; iedge < num_tree_edges; ++iedge) {
      global_tree_indices[iedge] = tree_indices[iedge].first;
    }
    const t8_gloidx_t global_tree_id = t8_cmesh_get_global_id (cmesh_from, itree);
    set_global_edge_ids_of_tree_edges (cmesh, global_tree_id, global_tree_indices, num_tree_edges);
