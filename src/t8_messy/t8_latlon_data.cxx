...
t8_latlon_new_chunk (const char *description, t8_locidx_t x_start,
                     t8_locidx_t y_start, t8_locidx_t x_length,
                     t8_locidx_t y_length, t8_locidx_t z_length, int *shape,
                     int num_tracers, int x_axis, int y_axis, int z_axis,
                     int level, double missing_value,
                     T8_LATLON_DATA_NUMBERING numbering)
{
  ...
  chunk->data =
    T8_ALLOC_ZERO (double, x_length * y_length * z_length * num_tracers);
 ...
 */
double
t8_latlon_get_dimension_value (int axis, double ****data, int x_coord,
...
int
t8_latlon_get_tracer_idx (t8_latlon_data_chunk_t * data_chunk,
                          const char *dimension, bool add_if_missing)
{
  int                 idx;

  /* search for dimension name */
  for (idx = 0; idx < data_chunk->tracer_names_size; ++idx) {
    if (strncmp (dimension, data_chunk->tracer_names + idx * BUFSIZ, BUFSIZ)
        == 0) {
      return idx;
    }
  }
...
void
t8_latlon_set_dimension (t8_latlon_data_chunk_t * data_chunk, char *tracer,
                         double ****data)
{
 ...
 */
void
t8_latlon_set_dimension_value (int axis, double ****data, int x_coord,
                               int y_coord, int z_coord, int dimension,
                               double value)
{
  switch (axis) {
  ...
  for (index = 0; index < num_elements; ++index) {
    ...
    if (x < data_chunk->x_length && y < data_chunk->y_length) {
      ...
      memcpy (data_new + index * element_length,
              data_chunk->data + old, element_length * sizeof (double));
    }
    else {
                     ...
                     t8_gloidx_t x_length_global, t8_gloidx_t y_length_global)
{
  ...
  t8_latlon_data_chunk_t *chunk =
    t8_latlon_new_chunk ("test_chunk", x_start, y_start, x_length, y_length,
                         1, shape, dimension,
  ...
  T8_ASSERT (numbering != T8_LATLON_DATA_MORTON);
  {
    ...
    int                 d1 =
      x_axis == 0 ? x_length : (y_axis == 0 ? y_length : 1);
    int                 d2 =
      x_axis == 1 ? x_length : (y_axis == 1 ? y_length : 1);
    int                 d3 =
      x_axis == 2 ? x_length : (y_axis == 2 ? y_length : 1);
