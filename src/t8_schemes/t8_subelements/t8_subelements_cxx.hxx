...
#ifndef T8_SUBELEMENT_CXX_HXX
#define T8_SUBELEMENT_CXX_HXX
...
typedef struct 
{
  ...
  /* stores transition cell information (default for non-subelements is 0 and for subelements it is != 0 - is therefore used as a is_subelement check)
     what type of subelement is stored here? Removing hanging nodes, subpatches, which parent cell type? */
  int                 subelement_type; // TODO: enum?
  /* For subelements number of type, for subpatches dimension (default for non-subelements is 0) */ //TODO: make pair out of cell_type and transition_type?
  int                 transition_type;
  /* stores subelement information (default for non-subelements is 0) */
  int                 subelement_id;
} t8_element_with_subelements;

struct t8_subelement_scheme_c: public t8_eclass_scheme_c
{
 public:
  ...
  /** Constructor. */
  t8_subelement_scheme_c (t8_eclass_scheme_c *scheme);

  ~t8_subelement_scheme_c ();
  ...
  */
   ...
   *                      id must fulfil 0 <= id < 'number of feaves in the uniform refinement'
   */
  virtual void
   ...
   * \param [in] level A refinement level.
   * \return The value of \ref t8_element_count_feaves if the input element
   *      is the root (level 0) element.
   *
   * This is a convenience function, and can be implemented via
   * \ref t8_element_count_feaves.
   */
  virtual t8_gloidx_t
...
/*------------------------Subelement specific----------------------------------*/
   ...
   */
  virtual void
  t8_element_to_transition_cell (const t8_element_t *elem,
                                                     int type,
                                                     t8_element_t
                                                     *subelements[]);
   ...
   * \param [in] source The element whose entries will be copied to \b dest.
   * \param [out] zero if no subelement, one if subelement
   */
  virtual int
   ...
   */
  virtual void
  t8_element_reset_subelement_values (t8_element *elem) const;
   ...
   */
  virtual void
  t8_element_copy_subelement_values (const t8_element * source, t8_element * dest) const;
   ...
   */
  virtual int
  t8_element_neighbor_is_sibling (const t8_element * elem,
   ...
   */
  virtual int
  t8_element_get_transition_type (const t8_element * elem) const;
  ...
  virtual int
  t8_element_scheme_supports_transitioning (void) const;

  /** Return 1 if the eclass scheme has an implementation for subelements, which is conformal. */
  virtual int
  t8_element_transition_scheme_is_conformal (void) const;
  ...
  */
  virtual int
  t8_element_find_neighbor_in_transition_cell (const
                                                                   t8_element_t
                                                                   *elem,
                                                                   const
                                                                   t8_element_t
