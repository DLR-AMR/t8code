...
t8_default_scheme_pyramid::element_MPI_Pack_size (const unsigned int count, sc_MPI_Comm comm, int *pack_size) const
{
  ...
  SC_CHECK_MPI (sc_MPI_Pack_size (1, MPI_INT8_T, comm, &datasize));
  singlesize += datasize;
                                               ...
                                               sc_MPI_Comm comm) const
{
  ...
  for (unsigned int ielem = 0; ielem < count; ielem++) {
    ...
    mpiret
      = sc_MPI_Unpack (recvbuf, buffer_size, position, &pyramids[ielem]->switch_shape_at_level, 1, MPI_INT8_T, comm);
    SC_CHECK_MPI (mpiret);
  }
}
