...
/**
 * TODO: COMMENT only works for from tet to tet or from pyra to pyra
 * 
 *
                            ...
                            t8_dpyramid_t *first_pyra_anc)
{
  ...
  if (tet->type != 0 && tet->type != 3) {
    ...
    T8_ASSERT (tmp_tet.type == 0 || tmp_tet.type == 3);
                      ...
                      t8_dpyramid_t *anc)
{
  ...
  if (t8_dpyramid_shape (pyra) == T8_ECLASS_PYRAMID) {
    ...
    anc->type = compute_type (pyra, level);
  }
  else {
    /* The shape may switch */
    T8_ASSERT (t8_dpyramid_shape (pyra) == T8_ECLASS_TET);
    int                 current_level = pyra->level;
    int                 type_at_level = pyra->type;
    ...
    while (type_at_level != 0 && type_at_level != 3 && current_level > level) {
      ...
      current_level--;
      type_at_level =
        compute_type_ext (pyra, current_level, type_at_level,
                          current_level + 1);
    }
    if (level == current_level) {
      ...
      t8_dpyramid_copy (pyra, &tmp_pyra);
      tmp_pyra.type = type_at_level;
      tmp_pyra.level = current_level;
                 ...
                 const t8_dpyramid_t *pyra2, t8_dpyramid_t *nca)
{
  ...
  t8_dpyramid_coord_t excl_or_x;
  t8_dpyramid_coord_t excl_or_y;
  t8_dpyramid_coord_t excl_or_z;
  ...
  if (t8_dpyramid_shape (pyra1) == T8_ECLASS_PYRAMID &&
    ...
    p1_type_at_level = pyra1_anc.type;
    p2_type_at_level = pyra2_anc.type;
