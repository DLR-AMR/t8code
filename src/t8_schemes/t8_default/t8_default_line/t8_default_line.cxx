                                          ...
                                          const int buffer_size, int *position, sc_MPI_Comm comm) const
{
  ...
  for (unsigned int ielem = 0; ielem < count; ielem++) {
    ...
    SC_CHECK_MPI (mpiret);
    mpiret = sc_MPI_Pack (&lines[ielem]->level, 1, MPI_INT8_T, send_buffer, buffer_size, position, comm);
    SC_CHECK_MPI (mpiret);
  }
}
                                            ...
                                            t8_element_t **elements, const unsigned int count, sc_MPI_Comm comm) const
{
  ...
  for (unsigned int ielem = 0; ielem < count; ielem++) {
    ...
    SC_CHECK_MPI (mpiret);
    mpiret = sc_MPI_Unpack (recvbuf, buffer_size, position, &(lines[ielem]->level), 1, MPI_INT8_T, comm);
    SC_CHECK_MPI (mpiret);
  }
}
