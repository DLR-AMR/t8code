...
/** Forward declaration of the simplex dimension struct. */
template <typename T>
struct simplex_dimension;

/** Specialization of the simplex dimension struct for triangles. */
template <>
struct simplex_dimension<t8_dtri_t>
{
  static constexpr std::size_t value = 2;
};

/** Specialization of the simplex dimension struct for tetrahedra. */
template <>
struct simplex_dimension<t8_dtet_t>
{
  static constexpr std::size_t value = 3;
};

/** Variable template for easy access to the simplex dimension. */
template <typename T>
inline constexpr std::size_t simplex_dimension_v = simplex_dimension<T>::value;
...
class t8_default_scheme_tri: public t8_default_scheme_common<T8_ECLASS_TRIANGLE, t8_default_scheme_tri> {
  ...
  */
  ...
  element_get_vertex_integer_coords (const TSimplexType *simplex, int vertex, int coords[]) noexcept
  {
    ...
    *   z--> x
    */
    constexpr bool is_tet = std::is_same_v<TSimplexType, t8_dtet_t>;

    int ei;
    [[maybe_unused]] int ej;
    /* Simplices always have dim + 1 corners */
    T8_ASSERT (0 <= vertex && vertex < (int) (simplex_dimension_v<TSimplexType> + 1));
  ...
  {
    int coords_int[simplex_dimension_v<TSimplexType>];
    /* Simplices always have dim + 1 corners */
    T8_ASSERT (0 <= vertex && vertex < simplex_dimension_v<TSimplexType> + 1);
    ...
    for (size_t dim = 0; dim < simplex_dimension_v<TSimplexType>; ++dim) {
   ...
   */
     ...
     * to divide them by the root length. */
      out_coords[0] /= (double) get_simplex_root_length (simplex);
      out_coords[1] /= (double) get_simplex_root_length (simplex);
      if constexpr (is_tet) {
        /* --begin tet logic-- */
        out_coords[2] /= (double) get_simplex_root_length (simplex);
        /* --end tet logic-- */
      }
    }
  }
