                      ...
                      int *position, sc_MPI_Comm comm)
{
  ...
  for (unsigned int ielem = 0; ielem < count; ielem++) {
    ...
    mpiret = sc_MPI_Pack (&elements[ielem]->type, 1, MPI_INT8_T, send_buffer, buffer_size, position, comm);
    SC_CHECK_MPI (mpiret);
    mpiret = sc_MPI_Pack (&elements[ielem]->level, 1, MPI_INT8_T, send_buffer, buffer_size, position, comm);
    SC_CHECK_MPI (mpiret);
  }
}
                        ...
                        const unsigned int count, sc_MPI_Comm comm)
{
  ...
  for (unsigned int ielem = 0; ielem < count; ielem++) {
    ...
    SC_CHECK_MPI (mpiret);
#endif
    mpiret = sc_MPI_Unpack (recvbuf, buffer_size, position, &(elements[ielem]->type), 1, MPI_INT8_T, comm);
    SC_CHECK_MPI (mpiret);
    mpiret = sc_MPI_Unpack (recvbuf, buffer_size, position, &(elements[ielem]->level), 1, MPI_INT8_T, comm);
    SC_CHECK_MPI (mpiret);
  }
}
