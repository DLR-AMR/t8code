...
struct t8_subelementtri_scheme: public t8_subelement_scheme_common<T8_ECLASS_TRIANGLE, t8_subelementtri_scheme>
{
 ...
 private:
                               ...
                               std::array<std::array<int, 2>, 3> &vertex_coords) const noexcept
  {
    ...
    std::array<int, 2> x0_coords_parent;
    underlying_scheme.element_get_vertex_integer_coords (this->subelement_to_standalone (subelement), 0,
                                                         x0_coords_parent.data ());
    std::array<int, 2> x1_coords_parent;
    underlying_scheme.element_get_vertex_integer_coords (this->subelement_to_standalone (subelement), 1,
                                                         x1_coords_parent.data ());
    std::array<int, 2> x2_coords_parent;
    underlying_scheme.element_get_vertex_integer_coords (this->subelement_to_standalone (subelement), 2,
                                                         x2_coords_parent.data ());
    ...
    */
    if (num_ones == 1) {
      // Binary representation of type is: ( hanging_face(0), hanging_face(1), hanging_face(2) ),
      // where hanging_face i is 1 if face i is hanging and 0 otherwise.
      const int hanging_face = (T8_ELEMENT_NUM_FACES[T8_ECLASS_TRIANGLE] - 1) - std::countr_zero (type);
      switch (hanging_face) {
      case 0:
    ...
    if (num_ones == 2) {
      unsigned copy_type = type;
      copy_type &= (copy_type - 1);
      const int hanging_face1 = (T8_ELEMENT_NUM_FACES[T8_ECLASS_TRIANGLE] - 1) - std::countr_zero (copy_type);
      const int hanging_face2 = (T8_ELEMENT_NUM_FACES[T8_ECLASS_TRIANGLE] - 1) - std::countr_zero (type);

      switch (hanging_face1) {
      case 0:
        if (hanging_face2 == 1) {
          vertex_coords[2][0] = 0.5 * (x1_coords_parent[0] + x2_coords_parent[0]);
          vertex_coords[2][1] = 0.5 * (x1_coords_parent[1] + x2_coords_parent[1]);
          ...
          if ((id == 1) || (id == 2)) {
            vertex_coords[1][0] = 0.5 * (x0_coords_parent[0] + x2_coords_parent[0]);
            vertex_coords[1][1] = 0.5 * (x0_coords_parent[1] + x2_coords_parent[1]);
          }
          if (id == 2) {
            vertex_coords[0] = x2_coords_parent;
          }
        ...
        if (hanging_face2 == 2) {
          ...
          if ((id == 0) || (id == 1)) {
            vertex_coords[1][0] = 0.5 * (x0_coords_parent[0] + x1_coords_parent[0]);
            vertex_coords[1][1] = 0.5 * (x0_coords_parent[1] + x1_coords_parent[1]);
          }
          if (id == 2) {
            vertex_coords[0] = x2_coords_parent;
          }
        ...
        break;
      case 1:
        T8_ASSERT (hanging_face2 == 2);
        if (id == 0) {
          vertex_coords[2] = x2_coords_parent;
        }
        if ((id == 1) || (id == 0)) {
          vertex_coords[0] = x1_coords_parent;
        }
