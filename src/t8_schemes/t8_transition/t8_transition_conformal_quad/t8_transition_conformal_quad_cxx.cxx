...

{
  ...
  for (iface = 0; iface < P4EST_FACES; iface++) {       /* Count the number of ones of the binary transition type. This number equals the number of hanging faces. */
    num_hanging_faces +=
      (pquad_w_sub->transition_type & (1 << iface)) >> iface;
  }
...
int
t8_subelement_scheme_quad_c::t8_element_neighbor_is_sibling (const
                                                             t8_element_t
                                                             *elem,
                                                             const int face)
  const
...
/* *INDENT-OFF* */
void
t8_subelement_scheme_quad_c::t8_element_get_sibling_neighbor_in_transition_cell (const t8_element_t
                                                                                 *elem, const int face,
...
t8_subelement_scheme_quad_c::t8_element_is_family (t8_element_t *const *fam) const
{
  ...
  /* If the first element of fam is no subelement we check the following elements of fam */
    ...
    if (pquad_w_sub_family[1]->transition_type != 0 ||
        pquad_w_sub_family[2]->transition_type != 0 ||
...
void
t8_subelement_scheme_quad_c::t8_element_vertex_reference_coords (const
                                                                 t8_element_t
                                                                 *t,
                                                                 int vertex,
                                                                 double
...
void
t8_subelement_scheme_quad_c::t8_element_vertex_coords (const t8_element_t
                                                       *elem, int vertex,
                                                       int coords[]) const
{
  const t8_quad_with_subelements *pquad_w_sub =
                                                            ...
                                                            t8_element_t *c[])
{
  ...
  int                 num_subelements =
    t8_element_get_number_of_subelements (type);
  ...
  for (sub_id_counter = 0; sub_id_counter < num_subelements; sub_id_counter++) {
    ...
    t8_element_copy_surround (q,
                              &pquad_w_sub_subelement[sub_id_counter]->p4q);
  }
}
                                                                    ...
                                                                    []) const
{
  ...
  T8_ASSERT (pquad_w_sub->subelement_id <
...
t8_subelement_scheme_quad_c::t8_element_equal (const t8_element_t *elem1, const t8_element_t *elem2) const{
  ...
  if(t8_element_get_subelement_id((const t8_element_t * ) elem1) == t8_element_get_subelement_id((const t8_element_t *) elem2)){
    return 1;
  }
else{
  return 0;
...
t8_subelement_scheme_quad_c::t8_element_debug_print (const t8_element_t *elem) const
{
  ...
  t8_productionf ("\n|------------ t8_element_debug_print: ------------|"
                  ...
                  "\n|    Level:               %i"
                  "\n|-------------------------------------------------|\n",
                  pquad_w_sub->transition_type, pquad_w_sub->subelement_id,
                  pquad_w_sub->p4q.x, pquad_w_sub->p4q.y,
                  (double) pquad_w_sub->p4q.x / (double) P4EST_ROOT_LEN,
                  (double) pquad_w_sub->p4q.y / (double) P4EST_ROOT_LEN,
...

{
  ...
  return ((pquad_w_sub->transition_type >= 0 &&
...
void
t8_subelement_scheme_quad_c::t8_element_to_string (const t8_element_t *elem, char *debug_string, const int string_size) const{
  SC_ABORT_NOT_REACHED();
}
#endif
