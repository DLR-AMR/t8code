...
t8_dprism_init_linear_id (t8_dprism_t * p, int level, uint64_t id)
{
  ...
  for (i = 0; i <= level; i++) {
    ...
    line_id +=
      ...
      sc_intpow64u (T8_DLINE_CHILDREN, i);
                        ...
                        int root_face)
{
  ...
  case 0:
    ...
    p->tri.y = q->x * T8_DTRI_ROOT_LEN / P4EST_ROOT_LEN;
    p->line.x = q->y * T8_DLINE_ROOT_LEN / P4EST_ROOT_LEN;
    break;
  case 1:
    p->line.level = q->level;
    p->tri.level = q->level;
    p->tri.x = q->x * T8_DTRI_ROOT_LEN / P4EST_ROOT_LEN;
    p->tri.y = q->x * T8_DTRI_ROOT_LEN / P4EST_ROOT_LEN;
    p->line.x = q->y * T8_DLINE_ROOT_LEN / P4EST_ROOT_LEN;
    break;
  case 2:
...
t8_dprism_successor (const t8_dprism_t * p, t8_dprism_t * succ, int level)
{
  ...
  if (prism_child_id == T8_DPRISM_CHILDREN - 1) {
    ...
    succ->line.x =
      (succ->
       line.x >> (T8_DLINE_MAXLEVEL - level + 1)) << (T8_DLINE_MAXLEVEL -
                                                      level + 1);
#endif
