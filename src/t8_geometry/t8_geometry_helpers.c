  ...
  case T8_ECLASS_PYRAMID:
    for (i_coord = 0; i_coord < num_coords; i_coord++) {
      ...
      * and then we interpolate via the height to the top vertex */
      ...
      t8_vec_diff_c_interface (tree_vertices + 4 * T8_ECLASS_MAX_DIM, out_coords + offset_domain_dim, vec);
      /* Add vector to base */
                                              ...
                                              const double *ref_coords, const size_t num_coords, double *out_coords)
{
  ...
  double vector[3];
  t8_vec_diff_c_interface (tree_vertices + T8_ECLASS_MAX_DIM, tree_vertices, vector);
...
t8_geom_get_tet_face_intersection (const int face, const double *ref_coords, double face_intersection[3])
{
  ...
  double vector[3] = { 0 };
  t8_vec_diff_c_interface (ref_coords, ref_opposite_vertex, vector);
...
t8_vertex_point_inside (const double vertex_coords[3], const double point[3], const double tolerance)
{
  ...
  if (t8_vec_dist_c_interface (vertex_coords, point) > tolerance) {
    return 0;
  }
  return 1;
}
...
t8_line_point_inside (const double *p_0, const double *vec, const double *point, const double tolerance)
{
  ...
  double b[3];
  /* b = p - p_0 */
  t8_vec_axpyz_c_interface (p_0, point, b, -1);
  ...
  */
  ...
  t8_vec_ax_c_interface (vec_check, x);
  if (t8_vec_dist_c_interface (vec_check, b) > tolerance) {
...
t8_plane_point_inside (const double point_on_face[3], const double face_normal[3], const double point[3])
{
  ...
  t8_vec_axpy_c_interface (point, pof, -1);
  /* Compute <x-p,n> */
  const double dot_product = t8_vec_dot_c_interface (pof, face_normal);
  if (dot_product < 0) {
