...
struct t8_geometry_cad: public t8_geometry_with_vertices
{
 ...
 private:
  ...
  /**
   * Maps points in the reference space \f$ \f$ [0,1]^3 \f$ \f$ to \f$ \mathbb{R}^3 \f$. Only for pyramid trees.
   * \param [in]  cmesh      The cmesh in which the point lies.
   * \param [in]  gtreeid    The global tree (of the cmesh) in which the reference point is.
   * \param [in]  ref_coords  Array of \a dimension x \a num_coords many entries, specifying points in \f$ [0,1]^\mathrm{dim} \f$.
   * \param [in]  num_coords  Amount of points of /f$ \mathrm{dim} /f$ to map.
   ...
   */
  void
  t8_geom_evaluate_cad_pyramid (t8_cmesh_t cmesh, t8_gloidx_t gtreeid, const double *ref_coords,
