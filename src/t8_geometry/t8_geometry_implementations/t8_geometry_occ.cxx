                                           ...
                                           const size_t num_coords, double out_coords[3]) const
{
  ...
  for (int i_edge = 0; i_edge < num_edges; ++i_edge) {
    ...
    if (edges[i_edge] > 0 || edges[i_edge + num_edges] > 0) {
      ...
      double scaling_factor_neigh_face_1 = t8_geom_get_scaling_factor_of_edge_on_face (
        i_edge, t8_edge_to_face[active_tree_class][i_edge][0], ref_coords);
      double scaling_factor_neigh_face_2 = t8_geom_get_scaling_factor_of_edge_on_face (
      ...
      for (int i_face_edge = 0; i_face_edge < 3; ++i_face_edge) {
        ...
        if (edges[i_tree_edge] > 0) {
          ...
          const double *curve_parameters = (double *) t8_cmesh_get_attribute (
            cmesh, t8_get_package_id (), T8_CMESH_OCC_EDGE_PARAMETERS_ATTRIBUTE_KEY + i_tree_edge, ltreeid);
          ...
          T8_ASSERT (edges[i_tree_edge] <= occ_shape_edge_map.Size ());
          curve = BRep_Tool::Curve (TopoDS::Edge (occ_shape_edge_map.FindKey (edges[i_tree_edge])), first, last);
          T8_ASSERT (!curve.IsNull ());
          ...
          double scaling_factor_neigh_face_1 = t8_geom_get_scaling_factor_of_edge_on_face (
            i_tree_edge, t8_edge_to_face[active_tree_class][i_tree_edge][0], face_intersection);
          double scaling_factor_neigh_face_2 = t8_geom_get_scaling_factor_of_edge_on_face (
            i_tree_edge, t8_edge_to_face[active_tree_class][i_tree_edge][1], face_intersection);
          double scaling_factor = scaling_factor_neigh_face_1 * scaling_factor_neigh_face_2;
