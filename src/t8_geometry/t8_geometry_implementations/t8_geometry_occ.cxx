                                           ...
                                           double out_coords[3]) const
{
  ...
  for (int i_edge = 0; i_edge < num_edges; ++i_edge) {
    ...
    if (edges[i_edge] > 0 || edges[i_edge + num_edges] > 0) {
      ...
      /* Save the edge vertices -> There is a function in t8_geometry_helpers for this, but it only works for hex */
      for (int i_edge_vertex = 0; i_edge_vertex < 2; ++i_edge_vertex) {
        for (int i_dim = 0; i_dim < 3; ++i_dim) {
          const int           i_tree_vertex =
            t8_edge_vertex_to_tree_vertex[active_tree_class][i_edge]
            [i_edge_vertex];
  ...
  for (int i_faces = 0; i_faces < num_faces; ++i_faces) {
    ...
    if (faces[i_faces] > 0) {
      ...
      /* Allocate some variables for later usage */
      ...
      for (int vertex = 0; vertex < 3; ++vertex) {
        for (int dim = 0; dim < 3; ++dim) {
          ref_face_vertex_coords[3 * vertex + dim]
            = t8_element_corner_ref_coords[active_tree_class]
      ...
      for (int i_face_edge = 0; i_face_edge < 3; ++i_face_edge) {
        ...
        if (edges[t8_face_edge_to_tree_edge[active_tree_class][i_faces]
                  [i_face_edge]] > 0) {
          ...
          /* Safe the surface parameters of both nodes on the edge */
          for (int i_edge_vertex = 0; i_edge_vertex < 2; ++i_edge_vertex) {
            for (int i_dim = 0; i_dim < 2; ++i_dim) {
              const int           i_tree_vertex =
                t8_edge_vertex_to_tree_vertex[active_tree_class]
          ...
          curve =
            BRep_Tool::Curve (TopoDS::Edge
                              (occ_shape_edge_map.FindKey
                               (edges[t8_face_edge_to_tree_edge
                                      [active_tree_class][i_faces]
