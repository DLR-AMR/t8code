    ...
    for (int i_edge = 0; i_edge < num_edges; i_edge++) {
      if (edges[i_edge] > 0) {
        ...
        for (size_t i_coord = 0; i_coord < num_coords; ++i_coord) {
          ...
          cad_handle->t8_geom_edge_parameter_to_face_parameters (edges[i_edge], *faces, num_face_nodes,
                                            ...
                                            const size_t num_coords, double *out_coords) const
{
  ...
  if (*faces > 0) {
    ...
    for (int i_edge = 0; i_edge < num_edges; ++i_edge) {
      if (edges[i_edge] > 0) {
        ...
        for (size_t coord = 0; coord < num_coords; ++coord) {
          ...
          cad_handle->t8_geom_edge_parameter_to_face_parameters (
    ...
    /* Retrieve surface */
    auto surface = cad_handle->t8_geom_get_cad_surface (*faces);
    ...
    for (int i_edge = 0; i_edge < num_edges; ++i_edge) {
      if (edges[i_edge] > 0 || edges[i_edge + num_edges] > 0) {
        ...
        if (edges[i_edge] > 0) {
          ...
          /* Retrieve curve */
          auto curve = cad_handle->t8_geom_get_cad_curve (edges[i_edge]);
      ...
      if (edges[i_edge] > 0 || edges[i_edge + num_edges] > 0) {
        ...
        * Therefore, we have to distinguish if the edge has a curve or surface linked to it. */
        ...
        if (edges[i_edge] > 0) { /* Check for linked curves */
          ...
          T8_ASSERT (edges[i_edge + num_edges] <= cad_handle->t8_geom_get_cad_shape_face_map ().Size ());
...
t8_geometry_cad::process_curve (const int edge_index, const double interpolated_curve_param) const
{
  ...
  auto curve = cad_handle->t8_geom_get_cad_curve (edges[edge_index]);
