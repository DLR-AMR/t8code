...
file(READ "cmake/thirdparty.json" DEPS_JSON)
# 2. Get the number of elements in the "thirdparty" array
string(JSON DEPS_COUNT LENGTH "${DEPS_JSON}" "thirdparty")
...
foreach(INDEX RANGE ${DEPS_RANGE})
    # Extract fields
    string(JSON DEP_NAME GET "${DEPS_JSON}" "thirdparty" ${INDEX} "name")
    string(JSON DEP_CMAKE_OPTION GET "${DEPS_JSON}" "thirdparty" ${INDEX} "depends_on_cmake_option")
    string(JSON DEP_TYPE GET "${DEPS_JSON}" "thirdparty" ${INDEX} "source" "type")
    string(JSON DEP_URL  GET "${DEPS_JSON}" "thirdparty" ${INDEX} "source" "url")
    string(JSON DEP_REF  GET "${DEPS_JSON}" "thirdparty" ${INDEX} "source" "ref")
    string(JSON DEP_SHALLOW  GET "${DEPS_JSON}" "thirdparty" ${INDEX} "source" "shallow")
