                                 ...
                                 [[maybe_unused]] const int num_elements, t8_element_t *elements[])
{
  ...
  for (int iface = 0; iface < num_faces; ++iface) {
    ...
    if (scheme->element_is_root_boundary (tree_class, elements[0], iface)) {
      ...
      const int *linked_geometries = t8_geometry_cad::get_tree_geometries (cmesh, cmesh_ltreeid);
...
main (int argc, char **argv)
{
  ...
  if (helpme) {
    ...
    t8_cmesh_init (&cmesh);
    t8_cmesh_from_msh_file (&cmesh, fp.c_str (), 0, sc_MPI_COMM_WORLD, dim, 0, cad || geometry);
